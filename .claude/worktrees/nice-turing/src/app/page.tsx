...
// â”€â”€ Env-driven Supabase client. Set in Vercel â†’ Settings â†’ Environment Variables:
//     NEXT_PUBLIC_SUPABASE_URL, NEXT_PUBLIC_SUPABASE_ANON_KEY, NEXT_PUBLIC_TENANT_ID
const SB_URL = process.env.NEXT_PUBLIC_SUPABASE_URL!;
const SB_KEY = process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY!;
if (!SB_URL || !SB_KEY) console.error('[supabase] Missing NEXT_PUBLIC_SUPABASE_URL or NEXT_PUBLIC_SUPABASE_ANON_KEY');
const supabase = createClient(SB_URL, SB_KEY);

// â”€â”€ Canonical status enums â€” DB stores UPPERCASE; never use lowercase variants
...
const ROOMS = [
  ...
  },
  {
    ...
    desc: 'Our crown jewel â€” an expansive suite for six with panoramic views and bespoke service.',
    rate: 9000,
    amenities: ['Complimentary Breakfast', 'TV', 'High-Speed Wi-Fi', '24/7 Room Service', 'Front Office'],
...
const AVAIL_ROOMS = [
  { label: 'Fountain Deluxe â€” à§³4,000 (2 Guests)', value: 'Fountain Deluxe', rate: 4000, guests: 2 },
  { label: 'Premium Deluxe â€” à§³4,500 (2 Guests)', value: 'Premium Deluxe', rate: 4500, guests: 2 },
  { label: 'Superior Deluxe â€” à§³5,000 (2 Guests)', value: 'Superior Deluxe', rate: 5000, guests: 2 },
  { label: 'Twin Deluxe â€” à§³6,000 (4 Guests)', value: 'Twin Deluxe', rate: 6000, guests: 4 },
  { label: 'Royal Suite â€” à§³9,000 (6 Guests)', value: 'Royal Suite', rate: 9000, guests: 6 },
];
...
export default function HotelFountainLanding() {
  ...
  async function submitBooking() {
    ...
    try {
      // 1 â€” Find existing guest by email, only create if not found
      ...
      // 2 â€” Create reservation; throw if it fails so guest sees real error, not fake success
  ...
  return (
    <>
      ...
      <nav className="gs" style={{ position: 'fixed', top: 0, left: 0, right: 0, zIndex: 100, display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '16px 60px', background: navScrolled ? 'rgba(7,9,14,.97)' : 'linear-gradient(180deg,rgba(7,9,14,.95) 0%,transparent 100%)', borderBottom: `1px solid ${navScrolled ? 'rgba(200,169,110,.15)' : 'rgba(200,169,110,.06)'}`, transition: 'all .4s' }}>
        ...
        <button className="mobile-menu-btn" onClick={() => setMobileMenu(!mobileMenu)} style={{ display: 'none', background: 'none', border: '1px solid var(--br)', color: 'var(--tx)', padding: '8px 12px', cursor: 'pointer', fontSize: 16 }}>â˜°</button>
      </nav>
        ...
        <div className="hero-left-panel" style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', padding: '140px 60px 80px', position: 'relative', zIndex: 2 }}>
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: 'rgba(200,169,110,.1)', border: '1px solid rgba(200,169,110,.25)', padding: '7px 14px', marginBottom: 32, width: 'fit-content' }}>
            ...
            <span className="gs" style={{ fontSize: 9, letterSpacing: '.2em', textTransform: 'uppercase', color: 'var(--tx2)' }}>
              <strong style={{ color: 'var(--gold)', fontWeight: 400 }}>{availCount ?? 'â€”'}</strong>&nbsp; Available Rooms Tonight
            </span>
          </div>
          <div className="gs" style={{ fontSize: 9, letterSpacing: '.25em', textTransform: 'uppercase', color: 'var(--tx3)', marginBottom: 20, display: 'flex', alignItems: 'center', gap: 16 }}>
            ...
            Dhaka, Bangladesh &nbsp;Â·&nbsp; Est. Since 2010
          </div>
          ...
          <p className="gs" style={{ fontSize: 14, lineHeight: 1.8, color: 'var(--tx2)', maxWidth: 420, marginBottom: 48, fontWeight: 300 }}>
            Nestled in the heart of Dhaka, Hotel Fountain offers a sanctuary of refined comfort â€” from our signature Fountain Deluxe rooms to the magnificent Royal Suite.
          </p>
      ...
      <div className="gs stats-bar" style={{ background: 'var(--dark2)', borderTop: '1px solid var(--br)', borderBottom: '1px solid var(--br)', padding: '28px 60px', display: 'flex', flexWrap: 'wrap' }}>
        {[{ num: '28', lbl: 'Premium Rooms' }, { num: 'à§³4,000', lbl: 'Starting Rate / Night' }, { num: '24/7', lbl: 'Concierge Service' }, { num: '4.8â˜…', lbl: 'Guest Rating' }].map((s, i, a) => (
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingTop: 16, borderTop: '1px solid var(--br)' }}>
                  <div>
                    ...
                    <div className="cg" style={{ fontSize: 26, color: 'var(--gold)', fontWeight: 300 }}>
                      à§³{room.rate.toLocaleString()} <span className="gs" style={{ fontSize: 10, color: 'var(--tx3)', letterSpacing: '.1em', fontWeight: 300 }}>/ night</span>
                    </div>
                  </div>
        ...
        <div className="exp-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 60, alignItems: 'start' }}>
          <div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
              {[
                { icon: 'ðŸ½ï¸', title: 'Restaurant', desc: 'Authentic Bangladeshi and international cuisine, 7amâ€“11pm.' },
                { icon: 'ðŸ’¼', title: 'Business Center', desc: 'Fully-equipped conference rooms and private offices.' },
                { icon: 'ðŸš—', title: 'Airport Transfer', desc: 'Complimentary pickup and drop-off for suite guests.' },
                { icon: 'ðŸŒ', title: 'High-Speed WiFi', desc: 'Fiber-optic internet throughout the entire property.' },
              ].map(f => (
        ...
        <div className="gc" style={{ maxWidth: 860, padding: 48, position: 'relative', zIndex: 1 }}>
          ...
          {selectedRoomInfo && (
            <div className="gs" style={{ marginBottom: 20, padding: '10px 16px', background: 'rgba(200,169,110,.06)', border: '1px solid var(--br)', fontSize: 12, color: 'var(--tx2)', display: 'flex', gap: 24 }}>
              ...
              <span>Rate: <strong style={{ color: 'var(--gold)' }}>à§³{selectedRoomInfo.rate.toLocaleString()}/night</strong></span>
          ...
          <button className="gb" disabled={searching} onClick={checkAvailability} style={{ width: '100%', padding: 15, fontSize: 11, letterSpacing: '.25em', ...glow }}>
            {searching ? 'Searching Availabilityâ€¦' : 'Check Availability â†’'}
          </button>
          {availResult && (
                    ...
                    {availResult.rooms.map((r: any, i: number) => {
                      ...
                      return (
                        <div key={r.id || i} style={{ padding: 16, background: 'rgba(200,169,110,.04)', border: '1px solid var(--br)', display: 'flex', flexDirection: 'column', gap: 8 }}>
                          ...
                          <div className="cg" style={{ fontSize: 20, color: 'var(--gold)' }}>à§³{(r.rate || cat.rate).toLocaleString()} <span className="gs" style={{ fontSize: 10, color: 'var(--tx3)' }}>/night</span></div>
        ...
        <div className="contact-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 80 }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 32 }}>
            {[
              ...
              { label: 'Hours', val: 'Front Desk: 24/7\nCheck-In: 12:00 PM Â· Check-Out: 12:00 PM' },
            ].map(item => (
              <div key={item.label}>
          ...
          <form onSubmit={e => e.preventDefault()} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            <textarea placeholder="Message or special requestâ€¦" className="fi gs" style={{ height: 120, resize: 'none', background: 'rgba(200,169,110,.04)' }} />
            <button type="submit" className="ob gs" style={{ alignSelf: 'flex-start' }}>Send Message â†’</button>
          </form>
        </div>
      </section>
      ...
      <section id="terms" className="sec" style={{ padding: '80px 60px', background: 'var(--dark)', borderTop: '1px solid var(--br)' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 32, flexWrap: 'wrap', gap: 16 }}>
          ...
          <button className="ob gs" style={{ fontSize: 9, padding: '9px 20px' }} onClick={() => setTncOpen(!tncOpen)}>
            {tncOpen ? 'Collapse â–²' : 'Read Full Terms â–¼'}
          </button>
        </div>
      ...
      <footer className="gs" style={{ background: 'var(--dark)', borderTop: '1px solid var(--br)', padding: '32px 60px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 16 }}>
        ...
        <div style={{ fontSize: 10, color: 'var(--tx3)', letterSpacing: '.12em' }}>Â© 2026 Hotel Fountain Â· Dhaka, Bangladesh Â· All Rights Reserved</div>
      ...
      {bookingModal.open && (
        <div className="mo" onClick={e => { if (e.target === e.currentTarget) setBookingModal({ open: false, room: null }); }}>
          <div className="gc afi" style={{ width: '100%', maxWidth: 480, padding: 48, position: 'relative' }}>
            <button onClick={() => setBookingModal({ open: false, room: null })} style={{ position: 'absolute', top: 16, right: 20, background: 'none', border: 'none', color: 'var(--tx3)', fontSize: 20, cursor: 'pointer' }}>âœ•</button>
            {bookStatus === 'success' ? (
              <div className="afi" style={{ textAlign: 'center', padding: '20px 0' }}>
                <div style={{ fontSize: 40, marginBottom: 16, color: 'var(--gold)' }}>âœ¦</div>
              ...
              <>
                ...
                <div className="gs" style={{ fontSize: 9, color: 'var(--gold)', letterSpacing: '.15em', textTransform: 'uppercase', marginBottom: 28, paddingBottom: 20, borderBottom: '1px solid var(--br)' }}>
                  {checkIn} â†’ {checkOut} Â· {guests} Â· à§³{(bookingModal.room?.rate || 0).toLocaleString()}/night
                </div>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                  {[
                    ...
                    { label: 'Phone Number', type: 'tel', key: 'phone', ph: '+880 â€” â€” â€”' },
                  ].map(f => (
                ...
                <button className="gb" disabled={bookStatus === 'sending' || !bookForm.name || !bookForm.email} style={{ width: '100%', padding: 15, marginTop: 28, fontSize: 10, ...glow }} onClick={submitBooking}>
                  {bookStatus === 'sending' ? 'Submittingâ€¦' : 'Confirm Reservation â†’'}
                </button>
                <p className="gs" style={{ fontSize: 10, color: 'var(--tx3)', textAlign: 'center', marginTop: 12 }}>
                  No payment required now Â· Our team will contact you within 2 hours to confirm
                </p>
              </>
            )}
          </div>
