...
- MINOR: notes-field first-save may show a one-time "— → text" diff if legacy text lives in `special_requests` not `notes` (harmless). Edit-email currently fires for admins too — flip to staff-only if owner wants less noise.
