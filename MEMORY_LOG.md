...
NIGHT-AUDIT EMAIL NOW MATCHES THE PRINTED CLOSING REPORT (2026-08-08, CODE EDITED, commit Windows-side):
- OWNER REPORT: the "Night Audit Closed" email disagreed with the Download-Report PDF for the SAME day (2026-08-07). Email: Total Collections ৳63,000 / Closing Balance ৳40,995 / Outstanding Dues ৳1,73,760 across 27. PDF: Total Collection ৳41,500 / Closing ৳19,495 / Dues ৳82,760 across 25.
- ROOT CAUSE 1 (৳21,500 collections gap): the email printed `night_audit_log.total_collections`, which `execute_nightly_audit()` computes with an EXCLUSION-ONLY filter (`type !~* 'balance carried forward'`). That is exactly the anti-pattern the house rules blacklist — it lets CHARGES count as revenue. Confirmed against the live ledger: 2026-08-07 had 5x "Stay Extension (+1 night)" = ৳21,500, a charge, counted as collection. The PDF uses the POSITIVE match (`REAL_PAY` + notBCF in Reports.jsx) and correctly excludes it. The same ৳21,500 then flowed into Closing Balance (token + collections − payouts).
- ROOT CAUSE 2 (dues gap): the email's shadowAudit counted EVERY non-cancelled reservation with `check_in <= auditDate`, including future/part-prepaid RESERVED+PENDING bookings, and used `discount` only (not the canonical `discount_amount` fallback) with no max(0) clamp. Canonical rule (src/lib/dues.js, owner decision 2026-06-12) is RECEIVABLES ONLY = CHECKED_IN/CHECKED_OUT, full live book, no check_in window. Verified: canonical = 25/৳82,760, old = 27/৳1,73,760.
- ROOT CAUSE 3 (latent): `dues: dueRows.slice(0,30)` but the email reported `f.dues.length` as the COUNT while `duesTotal` summed the full list — count silently under-states past 30 dues. Now `dueCount` (full) drives the count, `dues` stays the display slice.
- FIX (src/workflows/close-day-chain.ts only): the email no longer trusts the snapshot's derived money. shadowAudit() now recomputes Total Collection + the 5-way payment split from the live ledger using the PDF's own rules, and dues via the canonical receivables rule; the email renders the PDF's three cards (Financial / Payment Method / Operational). Opening Token + Payouts ARE still read from the snapshot — they're owner-entered cash-drawer values with no derivation to disagree about. A "PARITY BLOCK" comment at the top of the file marks the rules that MUST stay in lockstep with Reports.jsx. Also fixed the tx-day match to the PDF's `fiscal_day || created_at` coalesce (a bare `.eq('fiscal_day')` silently dropped rows written without one), and kept RAW/unclamped due for the ghost-BCF + negative-due integrity checks so overpays still trip them.
- VERIFIED against live 2026-08-07 data: new logic yields Total Collection ৳41,500, Cash ৳37,500, bKash ৳4,000, Nagad/Card/Bank ৳0, Closing ৳19,495, Dues 25 resv/৳82,760 — exact parity with the PDF the owner attached. tsc clean.
- FOLLOW-UP, SAME DAY (owner said go): `execute_nightly_audit()` RPC FIXED TOO — migration `night_audit_positive_payment_match` APPLIED TO PROD 2026-08-08 and committed as supabase/migrations/20260808_night_audit_positive_payment_match.sql. Two corrections: (a) `total_collections` now uses the SAME positive match as the PDF/email; (b) `carried_over_dues` now uses the canonical RECEIVABLES-ONLY rule (was counting future part-prepaid RESERVED/PENDING bookings; column is stored but never rendered, so zero visual blast radius). Verified post-apply for 2026-08-07: collections ৳41,500, carried dues ৳82,760 — exact PDF parity. Preserved verbatim: parameter DEFAULTS (CREATE OR REPLACE cannot drop them — ERROR 42P13 on the first attempt), SECURITY DEFINER, pinned `search_path=public, pg_temp`, and grants (postgres/service_role/crm_tenant EXECUTE; NO anon/authenticated).
- BLAST-RADIUS MEASURED before touching anything (60 closed days): stored vs correct differs on 46 days, ৳562,000 total. That gap splits into TWO different things — ৳376,500 across 40 days from the CHARGE BUG (Stay Extension +1/+2 nights ৳438,500 lifetime, Restaurant, [VOID-DUP] rows), and ৳185,500 across 22 days from POST-CLOSE DRIFT (back-dated payments + the 2026-07-30 overpaid-reconcile TXs landing on already-closed days). The drift component is arguably CORRECT behaviour: `total_collections` is a snapshot "locked at close" (that is literally the Reports StatCard's sub-label).
- HISTORY NOT REWRITTEN (deliberate): the 46 historical rows keep their closed values, so Reports' "Closed Collection · locked at close" still shows the old figure for past days (e.g. ৳63,000 on 2026-08-07) while the printed PDF shows ৳41,500. A full recompute is possible but CANNOT separate the two components — the tx set as-at-close is not recoverable (no tx edit history), so any backfill necessarily converts that column from "as at close" to "as of now". Backup exists: `_backup_night_audit_log_2026_08_08` (60 rows). **OWNER DECISION 2026-08-08: LEAVE HISTORY AS-IS** — closed books are not rewritten retroactively; only forward closes use the corrected math. Do NOT backfill night_audit_log without re-asking. Consequence to expect (not a bug): for days closed BEFORE 2026-08-08, the Reports "Closed Collection" card and a re-printed PDF will disagree; from the first close after this date they agree by construction.
- app/.well-known/workflow/v1/step/route.js is a GENERATED, gitignored artifact — do not hand-edit; it rebuilds from close-day-chain.ts.

STRICT-CSP NONCE MISSING ON NEXT'S OWN HYDRATION SCRIPTS — headers() read restored (2026-07-30, CODE EDITED on `main`; ships via Windows-side push — verify Vercel build GREEN + hard-refresh /crm, /admin, /lumea, /settings, /billing, /invoice):
- TRIGGER: owner still saw fresh CSP console errors on `/crm` AFTER the Vercel-telemetry fix below shipped — two DIFFERENT inline-script hashes (`sha256-zvmr...`, `sha256-n8EGvd...`), not the Vercel stub hashes from that fix. Both errors pointed at the `/crm` document itself, not a bundled `.js` file.
- ROOT CAUSE: the 2026-07-14 perf refactor (`8fffa8e`) removed `const nonce = (await headers()).get('x-nonce')` from the shared root `app/layout.tsx` so public marketing pages could static-render (that headers() call forces dynamic SSR on every route). What wasn't accounted for: reading the nonce via `headers()` is ALSO what makes Next.js thread the active per-request nonce into its OWN internal inline scripts (flight/hydration payload `<script>` tags) — `export const dynamic = 'force-dynamic'` alone does NOT do this. Every strict-CSP layout (`app/crm`, `app/admin`, `app/lumea`, `app/settings`) already had `force-dynamic` but none called `headers()`, and `/billing` + `/invoice` had no layout.tsx at all (inherited straight from the now-headers()-less root). Net effect: since 07-14, Next's own inline scripts on every one of these 6 surfaces have been silently missing the CSP nonce.
- FIX (6 files): added `await headers()` (return value unused — the call itself is what matters) to `app/crm/layout.tsx`, `app/admin/layout.tsx`, `app/lumea/layout.tsx`, `app/settings/layout.tsx` (all converted to async). Created `app/billing/layout.tsx` and `app/invoice/layout.tsx` (didn't exist) with the same minimal pattern — noindex metadata + force-dynamic + headers() read, mirroring the existing crm/admin/lumea/settings shape.
- VERIFY: all 6 files pass `typescript.transpileModule` syntax check + NUL-byte/truncation check (F:-mount corruption history) in the sandbox. NOT run through full `next build` here — Vercel build is the gate. Post-deploy: hard-refresh each of /crm, /admin, /lumea, /settings, /billing, /invoice and confirm the "Executing inline script violates CSP" console errors are gone.

STRICT-CSP INLINE-SCRIPT FIX — Vercel telemetry scoped to public (site) (2026-07-30, CODE EDITED on `main`; ship via push-csp-fix.bat / auto-PR — verify Vercel build GREEN + hard-refresh /crm):
- TRIGGER: owner console error on a strict page — `Executing inline script violates ... script-src 'self' 'nonce-...' https://connect.facebook.net https://www.googletagmanager.com https://cdn.cookiehub.eu ... a hash ('sha256-n8EGvd+f0qUCWGy8Y7UVeKDrBiP6td4UjWKroJeastE=') or a nonce is required`.
- ROOT CAUSE: `<SpeedInsights/>` + `<Analytics/>` were in the SHARED root `app/layout.tsx`. Both `@vercel/analytics`@^2 and `@vercel/speed-insights`@^2 inject an INLINE bootstrap stub (window.va/window.si queue) — NOT an external /_vercel/* file as the old root-layout comment claimed — and neither v2 component exposes a `nonce` prop (verified: "nonce" appears nowhere in either package). On public pages (static `unsafe-inline` CSP) the stub runs; on the strict per-request NONCE CSP subtrees (middleware STRICT_PREFIXES = /crm /api /admin /lumea /settings /billing /invoice) the un-nonceable inline stub is blocked.
- FIX (2 files): removed both components (+imports) from `app/layout.tsx`; added them to `app/(site)/layout.tsx` (public marketing pages only), where the existing static `unsafe-inline` CSP permits the inline stub. They're client components with no headers() call, so (site) stays STATIC (no TTFB regression). Route groups audited: every HTML route is either public/static (`(site)`, `/chat`) or strict/nonce — nothing else renders these components now (grep-confirmed only (site)/layout.tsx references them).
- TRADE-OFF (owner FYI): Vercel Web Analytics + Speed Insights no longer load on the authed staff surfaces (/crm etc.) — page-traffic analytics there is low-value, and this keeps the strict CSP clean (did NOT weaken it with a hash/unsafe-inline). If CRM perf telemetry is wanted back, the alternative is hash-allowlisting the two inline stubs in buildCsp() — brittle (breaks on every @vercel version bump), so not chosen.
- VERIFY: both files pass isolated /tmp transpileModule (dodges F:-mount sandbox-tsc truncation). NOT run through full next build here — Vercel build is the gate. Post-deploy: hard-refresh /crm and confirm the CSP console error is gone; confirm public pages still report to Vercel Analytics.

GUEST-INSERT 23505 HARDENING — guests_unique_real_email (2026-07-28, CODE EDITED on `main`; ships via auto-PR pipeline / Windows-side push — verify Vercel build GREEN):
- TRIGGER: full-stack health check. Everything green (Supabase ACTIVE_HEALTHY, last 20 Vercel prod deploys READY, crons clean, 0 advisor ERRORs) EXCEPT one recent Postgres error: `duplicate key value violates unique constraint "guests_unique_real_email"`. Index = `CREATE UNIQUE INDEX ... ON guests (email, tenant_id) WHERE email IS NOT NULL AND email <> ''` (partial, case-SENSITIVE on raw email text). guests table itself clean (1,765 rows, 0 real-email dups, 0 case-variant dups) — the constraint was doing its job; the bug was write-path logic doing raw INSERTs.
- SHADOW AUDIT of all guest-write paths: `app/api/crm/reservation` = SAFE (uses guest_ids/guest_name, no direct guests insert). `booking-webhook` (OTA) = find-or-create, low risk. Two paths were wrong:
  - `app/api/crm/guest/route.ts` create (THE logged error): raw `db.from('guests').insert(payload)`, no email normalization, no conflict handling → 23505 → generic 500 "Could not save guest." when staff add a guest whose email already exists for the tenant. FIX: (1) lowercase email on write — `email: (s(body.email)||'').toLowerCase()||null` — aligns with the unique index + /api/book normalization; (2) map the duplicate to a friendly 409 ("A guest with this email already exists — search for them and edit instead"), mirroring the existing delete-branch FK-violation 409 pattern.
  - `app/api/book/route.ts` (public booking, MONEY path): already normalizes email (lowercase+trim, L70) and looks up before insert, but a returning-guest / double-submit race could still trip 23505 between lookup and insert → 500 on a booking. FIX: on `create.status===409 || /23505|guests_unique_real_email|duplicate key/` → re-lookup by email+tenant and REUSE the existing guest id; only throw if the re-lookup also finds nothing. A website booking never 500s on this again.
- VERIFY: both files pass an ISOLATED transpileModule syntax check (copied to /tmp first — dodges the known F:-mount sandbox-tsc truncation false positives). DB re-scan confirmed 0 case-variant duplicate emails, so the new lowercase normalization is safe against existing data. `git status` shows only these 2 files modified, on `main`. NOT run through full `next build` here (45s sandbox cap) — Vercel build is the real gate.
- NOT CHANGED (deliberate): the CRM guest route now lowercases email but legacy rows keep original casing (harmless — no dups exist). Did not add ON CONFLICT upsert to the CRM create path on purpose: a manual "Add Guest" should surface the collision (409) so staff find/edit the real record rather than silently overwriting it.

HOMEPAGE LCP: IDLE-DEFERRED domMax FETCH + ROOMCARD IMAGE SIZING (2026-07-21c, CODE EDITED, commit Windows-side, NOT build-verified locally — see note):
- TRIGGER: live PSI re-check after the two globals.css/Tailwind splits above showed FCP improved (3.7s->2.8s) but mobile LCP barely moved (6.8s->6.7s). LCP breakdown: TTFB 0ms, Element render delay ~2,310ms, no resource-load-delay/duration rows — confirmed via PSI's "LCP request discovery" audit (passed on Mobile, no fetchpriority complaint) that the LCP element IS the hero image and IS already discovered/prioritized correctly. So the 2.3s+ gap is main-thread contention (PSI: "Avoid long main-thread tasks — 4 long tasks", "Reduce unused JavaScript — 179 KiB") delaying paint/composite of an already-ready frame, not a network-fetch problem.
- INVESTIGATED AND REJECTED: swapping framer-motion's LazyMotion `domMax` -> smaller `domAnimation` (the lever flagged in [[perf_mobile_res_investigation_2026_07_19]] as "eager framer core", ~179KiB-class). Independently re-verified via grep: `layoutId` IS genuinely used (Navbar.tsx `nav-active`, RoomsCatalog.tsx `room-filter-active`), and Navbar renders on every page — domAnimation would break the nav's active-pill animation site-wide. Confirms the prior owner-level "don't touch blindly" call was correct; did NOT redo this swap.
- SHIPPED FIX 1 (src/components/site/MotionProvider.tsx): `loadFeatures()` used to call `import("./motion-features")` immediately on LazyMotion mount (i.e. right at hydration) — the chunk's fetch+parse+exec landed inside the LCP window even though the hero itself uses zero framer-motion. Wrapped the same import in `requestIdleCallback(..., {timeout:2000})` (setTimeout(200) fallback for Safari, which lacks rIC). Same feature set still loads (domMax unchanged, layoutId still works) — only the *timing* moved to browser-idle, after the critical paint. `m.*` elements already render inert-but-visible before features attach (LazyMotion's own progressive-enhancement contract), so this is a pure timing shift, not a behavior change.
- SHIPPED FIX 2 (src/components/site/RoomCard.tsx): PSI "Improve image delivery" flagged Royal Suite/Premium Deluxe room images served at the 750px Next/Image bucket for a card rendering at 262–356px on the homepage's mobile carousel (`w-[76%] max-w-[300px]` per app/(site)/page.tsx) — the old `sizes="(max-width: 768px) 100vw, ..."` hint was wrong for that layout (RoomCard is shared with /rooms' RoomsCatalog, which IS full-width/`grid-cols-1` below the `sm:` 640px breakpoint). Added a `(max-width: 640px) 340px` bucket ahead of the existing ones — fixes the homepage carousel's over-fetch; /rooms mobile keeps requesting essentially the same bucket (Next rounds 340 up to its next imageSizes step, close to the card's real ~350-390px width there) so no visible quality loss expected.
- NOT BUILD-VERIFIED: same structural sandbox limit as the CSS entries below — this Cowork sandbox cannot complete a Tailwind/Next compile within its 45s bash cap on the F:\ mount. Both diffs are small, additive, and mechanically checked (UTF-8, brace/paren balance, no NUL bytes) but have NOT run through `next build`/`tsc` here. Vercel's build is the real gate — check the deploy log after push before trusting this shipped clean.

CRM TAILWIND UTILITY CLASSES ALSO SCOPED OUT OF globals.css (2026-07-21b, follow-up to the entry below - CODE EDITED, commit Windows-side):
- Moving the hand-written .crm-root CSS only cut globals.css's compiled output ~141KB->~119KB (confirmed live via curl after deploy) - the remaining ~98.7KB is Tailwind's own auto-generated utility classes (bg-*, flex, p-4 etc), which v4's zero-config auto-scan pulls from EVERY source file in the repo into the one shared stylesheet regardless of route, same root cause as before.
- CHANGE: app/globals.css gets 3 `@source not` excludes (src/components/*.jsx direct children, app/crm, app/billing) right after `@import "tailwindcss"` - verified via grep that none of app/(site)/admin/settings/lumea/invoice reference any file those globs match. src/components/crm-theme.css gets its OWN plain unscoped `@import "tailwindcss"` (full auto-scan, deliberately NOT source(none)-restricted) as a safety net: even if an exclude on the globals.css side is wrong or Tailwind's `@source not` has an edge case, crm-theme.css independently regenerates everything CRM needs from a full scan - failure mode is "smaller savings," never "missing CRM styles." Checked for and ruled out the one real footgun with this design (a CRM file using a Tailwind utility whose value only exists via app/globals.css's custom `@theme inline` tokens, e.g. `bg-background`/`text-foreground`/`text-neon-teal`/`rounded-xl2` - crm-theme.css's independent compile wouldn't know those custom tokens) - grep confirmed zero such usage anywhere under src/components (non-site) or app/crm.
- COULD NOT TEST-BUILD: this sandbox's node_modules has no Linux-built `lightningcss` native binary (project installs from Windows), so neither `next build` (>45s sandbox cap, unrunnable) nor a standalone `@tailwindcss/postcss` compile could run here. This change is UNVERIFIED beyond docs-reading + the safety-net design above - Vercel build is the real gate, and a manual visual pass over a few /crm pages (Dashboard, Billing, Reports) post-deploy is recommended once, specifically checking nothing looks unstyled/default-browser-font. If `@source not` turns out flaky (a documented community-reported risk), the worst outcome is globals.css stays at ~119KB - not a regression.
- LEFTOVER: `__tw_test.mjs` scratch file in the repo root (untracked, sandbox couldn't delete it - same file-mount permission issue as `.git/*.lock`) - safe to delete via PowerShell, was never staged.

CRM CSS EXTRACTED FROM globals.css - PUBLIC-SITE RENDER-BLOCKING FIX (2026-07-21, CODE EDITED via tinycss2 split script - commit must run Windows-side):
- TRIGGER: owner pasted a live PageSpeed Insights report for fountainbd.com mobile - Performance 65, LCP 6.8s, FCP 3.7s. This is the SAME "real lever" flagged but not yet fixed in [[perf_mobile_res_investigation_2026_07_19]] ("138KB render-blocking CSS + eager framer core... DON'T blind-split globals.css").
- ROOT CAUSE (confirmed via curl on the live prod HTML head): app/globals.css compiles to ~140KB across 2 render-blocking <link rel=stylesheet> tags. app/layout.tsx (ROOT layout, wraps EVERY route) imports globals.css once, so every route pays for its full contents. globals.css's `.crm-root { ... }` block (~580 of 721 source lines) is the ENTIRE modern-CRM "Aurora/Orbix" Ivory/gold theme - already comment-marked "Scoped under .crm-root so it never affects the public site" (visual isolation via selector specificity) but never byte-isolated (still shipped to every public pageview regardless).
- VERIFIED before touching anything: grep confirmed `.crm-root` wrapper div is rendered ONLY by src/components/Layout.jsx (consumed by app/crm/layout.tsx + app/billing/page.jsx). Zero references to `.crm-root`/`.iv-*`/`.gb`/`.ob`/`.fi`/`.rc`/`.ef`/`.st`/`.mo`/`.cg`/`.gs`/`.bell-drop-item` anywhere under app/(site), app/admin, app/settings, app/lumea, app/invoice.
- CHANGE: wrote a tinycss2-based Python script (parses top-level CSS rules, classifies each by selector/keyframe-name against a verified CRM marker list, handles @media wrappers by scanning nested content) to split app/globals.css into itself (17.7KB, down from 54.5KB source / was ~140KB compiled) + NEW src/components/crm-theme.css (37.5KB), imported only in src/components/Layout.jsx. Verified LOSSLESS: parsed both outputs back and confirmed the exact same 283 top-level rules exist across the two files as existed in the original (0 missing, 0 duplicated) - this is the "not blind" version of the split the 07-19 investigation was cautious about.
- NOT TOUCHED: public/crm-src.jsx (legacy monolith) has its own fully separate inline `<style>{CSS}</style>` system, unrelated to globals.css entirely.
- VERIFY: NUL=0 + UTF-8 clean on all 3 changed files (app/globals.css, src/components/crm-theme.css, src/components/Layout.jsx); brace-balanced. Could NOT run a full local `next build` - sandbox bash caps every call at 45s and cannot persist a background process across calls (confirmed empirically), so the Next.js production build (which routinely takes >45s) is unrunnable here. Gate = Vercel build (push, then check deployment state) + a follow-up PageSpeed Insights re-run on fountainbd.com to confirm the LCP/CSS-payload win empirically. If Vercel build fails, the most likely cause is a class name used somewhere the grep sweep missed - check the build log's CSS-related errors first.

B2B LEADS PIPELINE RECONNECTED (2026-07-21, CODE EDITED via Python string-replace + rebuild - commit must run Windows-side, see push note below):
- CONTEXT: owner asked for a sales/B2B CRM; investigation found NO such thing exists in the live /crm Next.js app (b2b_partners is a dangling permission stub, zero component behind it). The DB layer was already fully built and live (corporate_leads, swarm_leads, outreach_log, promote_swarm_lead() trigger) - only the UI was missing. Two fully-built React components already existed in public/crm-src.jsx but were DEAD CODE with zero call sites: LeadGenSwarmPanel (AI Scout/Analyst/Outreach agents -> swarm_leads) and LeadPipelinePage_REMOVED (deal-stage Kanban -> corporate_leads/outreach_log). ROLES already listed 'leads' in owner/manager .pages - the nav wiring had clearly been ripped out at some point.
- OWNER DECISION: revive the old monolith page (NOT build fresh in the live /crm app) + combine BOTH components into one "full funnel" page ("Yes - full funnel (Recommended)").
- CHANGE: public/crm-src.jsx - (1) renamed LeadPipelinePage_REMOVED -> LeadPipelinePage; (2) added a 6th 'pipeline' tab to LeadGenSwarmPanel's existing tab bar (dashboard/scout/analyst/outreach/leads/pipeline) rendering <LeadPipelinePage/>; (3) added {id:'leads',ico:'🎯',label:'B2B Leads',sect:'STRATEGY'} to NAV_ITEMS (STRATEGY section now leads->council); (4) added leads:'B2B Leads' to PAGE_TITLES; (5) added {cur==='leads' && <LeadGenSwarmPanel toast={toast}/>} to the content dispatch. Rebuilt via npm run build:crm - Babel/Terser clean, crm-bundle.js ends with the ReactDOM.createRoot bootstrap (no truncation), crm.html still has crm-bundle.js/id="root"/</html>.
- VERIFY: host Read + NUL=0 + iconv UTF-8 clean before build; grep-confirmed all 6 anchors landed and LeadPipelinePage_REMOVED has zero remaining references (bundle + source). LeadPipelinePage is fully self-contained (no props, uses only file-global db()/dbPatch()/TENANT/_HLOC) so the rename+mount was mechanical - no logic changed inside either component.
- NOT YET COMMITTED: only public/crm-src.jsx, public/crm-bundle.js, public/crm.html changed (cache-bust version bump). Push from Windows PowerShell per repo rule (sandbox .git/*.lock is Windows-owned).

STRUCTURED-DATA ENTITY GRAPH PASS (2026-07-17, CODE EDITED - ships via auto-PR pipeline; verify Vercel build GREEN):
- TRIGGER: GSC "now collecting impressions for fountainbd.com" email (2026-07-14 first-impression). Site already had titles/meta/canonical/OG + per-page schema (Hotel on all pages, HotelRoom+Offer on /rooms, Breadcrumb on rooms/services/contact/faq, FAQPage on /faq) from the 07-14 pass. Remaining on-page gap = entity graph, targeting the OPEN "zero local-pack visibility / weak entity recognition" finding.
- CHANGE 1 app/(site)/layout.tsx hotelSchema: added `"@id":"https://fountainbd.com/#hotel"` (stable entity anchor) + local signals `numberOfRooms:28`, `currenciesAccepted:"BDT"`, `hasMap` (coords 23.82924608574381,90.4162137758988), `openingHoursSpecification` Mo-Su 00:00-23:59 (24/7 reception, mirrors CONTACT.frontDesk + FAQ). Hotel schema still emits on EVERY (site) page (layout-level) - intentional.
- CHANGE 2 app/(site)/page.tsx (home): NEW Organization (#organization: logo icon-192, sameAs, reservations contactPoint areaServed BD, lang en/bn) + WebSite (#website, publisher->#organization) via shared JsonLd component. WebSite has NO SearchAction (no site-search endpoint exists - a potential-action to a dead URL would be invalid). All three nodes cross-link by @id = one entity graph, home page only.
- DELIBERATELY STILL SKIPPED: aggregateRating + starRating (owner positioning call, unchanged from 07-14; no on-page review widget yet). numberOfRooms=28 matches visible content (STATS "28 Premium Rooms" + /rooms "28 Rooms") NOT the CM ledger's 29 - schema must mirror the PAGE.
- VERIFY: host Read confirms both files complete (layout closes L53 hotelSchema, page closes L222); NUL=0; iconv UTF-8 clean (Taka priceRange line intact). Sandbox esbuild/cp gave FALSE truncation (documented F:-mount artifact) - authoritative gate = host Read + Vercel build. Post-deploy: Google Rich Results Test on / and /rooms.

DIGITAL AUDIT + ON-PAGE SEO CRITICAL FIXES (2026-07-14, CODE EDITED - NOT yet committed; push Windows-side or via auto-PR pipeline):
- AUDIT (live-verified): Cloudflare-managed robots.txt DISALLOWS GPTBot/ClaudeBot/Google-Extended/CCBot/meta-externalagent sitewide + Content-Signal ai-train=no -> negates the entire AEO/FAQ investment; ChatGPT/Claude/Gemini cannot read fountainbd.com. FIX IS CLOUDFLARE DASHBOARD (AI Crawl Control), NOT app/robots.ts - app code is correct. OWNER ACTION PENDING. Other findings: GBP 4.1/349 healthy (15K views/mo) but photos stale 143d and ZERO local-pack visibility for "hotel near dhaka airport" (peers 691-1,400 reviews); branded SERP poisoned by near-empty OTA profiles (MakeMyTrip 1.0/5, Agoda 2.8/10); TWO Facebook pages split 22.5K followers (canonical = @hotelfountain0806); IG 41 followers; Google Ads paused w/ stale Ramadan creative.
- CODE SHIPPED (10 files): metadataBase in app/layout.tsx; canonical + full openGraph (og:image from /public/images) + twitter card on all 5 public pages (home/rooms/services/contact/faq); homepage H1 gains sr-only "Boutique Hotel in Nikunja-02, Dhaka, 8 minutes from HSIA" (visual hero untouched); /crm "Staff Login" REMOVED from Navbar (desktop+mobile) + Footer (staff bookmark directly); src/lib/site.ts new SOCIALS const -> footer links + Hotel schema sameAs, new GOOGLE_REVIEW_URL (maps?cid=16595962154219546266 - TODO owner: swap for g.page/r short link from GBP "Ask for reviews").
- NEW src/lib/reviewRequest.ts - INERT (nothing imports it): reviewRequestWhatsApp() + buildReviewRequestEmail(). Reason: audit assumed a guest payment-confirmation email exists - IT DOES NOT (only owner close-day/dues alerts, staff OTP, SaaS-lead activation). Wire into checkout flow only after owner approves automated guest messaging.
- DELIBERATELY SKIPPED: schema starRating - star class conflicts across OTAs (TripAdvisor 3-star vs MMT/Goibibo 4-star); owner positioning call required first (also: one canonical "from" price - FB says 3,500, site 4,000, homepage card 5,000).
- REPAIR: Navbar.tsx had 331 trailing NUL bytes at EOF (write_tool_nul_padding family) - stripped via tr, re-verified.
- VERIFY: all 10 files NUL=0, iconv UTF-8 clean (Taka/em-dash intact), host Read confirms every file complete (sandbox esbuild shadow copies gave FALSE truncation errors - the documented F:-mount truncated-read artifact; do not trust sandbox cp/esbuild for these files). Authoritative gate remains host tsc + Vercel build. outputs/shadow/* copies are disposable junk.

COOKIEHUB SCROLL-LOCK BUG - SITE UNSCROLLABLE IN BANGLADESH (2026-07-14, CODE EDITED same pass):
- SYMPTOM: live fountainbd.com pages cannot scroll at all (owner report + reproduced in Chrome MCP).
- ROOT CAUSE: CookieHub region g0 (consent not required, e.g. BD) hides its root (.ch2 display:none) but its center dialog had already set body style="overflow:hidden" and the lock is never released -> invisible modal freezes every visitor. Verified live: #ch2-dialog ch2-visible aria-modal=true at 0x0, root display:none, body inline overflow:hidden.
- FIX app/layout.tsx cookiehub-init injector: after cookiehub.load(), a 10s/250ms interval releases body overflow ONLY IF (.ch2 root is display:none) AND (no OTHER visible aria-modal dialog owns the lock) - so a legit open ReservationModal is never unlocked. Fires once, self-clears at 40 ticks. Proven against the live broken page via console (scroll restored, scrollY 867).
- NOTE: this likely explains poor engagement/bounce on the public site for ALL Bangladeshi visitors since the CookieHub injector shipped (2026-07-10 perf pass). Check owner CookieHub dashboard: consider disabling the center-dialog for no-consent regions entirely.

 (2026-07-05, CODE EDITED — push via RUN_PUSH_PERF2.bat):
- CONTEXT: after the public-site fix landed (`/` 37→100, all core /crm routes 100), the only Speed-Insights routes still <90 were `/crm/marketing` (55, ~19 samples) and `/crm/reports` (78, ~85 samples) → these dragged the US country score to 60. Low sample counts = noisy p75, but a real O(n²) bug existed and was fixed.
- ROOT CAUSE (`src/components/Reports.jsx` `Daily`): `collectedFor(r)` and `_txTimeFor(r)` each scanned the ENTIRE `txs` array for every reservation, and were called inside `res.filter`/`.map` (payOnly, _movesColl) AND inside the movement sort comparator (`timeOf`→`_txTimeFor` per comparison) → O(res×txs) and O(n·log n·txs). ALL of it lives in the component body (not memoized), so it re-ran on every render incl. every keystroke in the close-day token/payout fields. Prime INP driver.
- FIX: added a memoized `_txByRes = Map<reservation_id, txs[]>` (built once per `txs`, pre-filters `notBCF`, skips null reservation_id). `collectedFor`/`_txTimeFor` now filter that reservation's short list + apply the SAME per-call date filter and sum → mathematically identical (Shadow-Audited: notBCF+reservation_id pre-applied, null-res-id excluded in both paths, order irrelevant for sum, .sort() makes list order moot). Also `Monthly`/`Yearly` bars: replaced the per-day/per-month full-txs scan (O(days×txs)/O(12×txs)) with a single memoized `byDay`/`byMonth` sum bucket (identical sums). `src/components/Marketing.jsx`: added `decoding="async"` to the poster `<img>` (trivial; images already lazy).
- FOLLOW-UP — /crm/reports DESKTOP CLS 0.43 (Poor) fixed same day: after the INP fix (INP→192ms good), the remaining Poor metric was Cumulative Layout Shift. CAUSE: on cold load the Daily Movements table showed a single "Loading…" row and the Outstanding Dues table had NO loading state at all → when data arrived both jumped from ~0 to N rows (dues up to 200), shoving the Closing Ledger card far down = big shift. FIX: render loading SKELETON rows (6 for Movements, 8 for Dues) that reserve realistic height so the fill-in doesn't move the cards below. Purely presentational, only shown when `loading` (i.e. no snapshot cache); cached revisits render data immediately as before. No financial logic touched.
- RISK NOTE: Reports.jsx is ghost-bleed/dedupe-sensitive (many CLAUDE.md rules). Changes are pure perf — no movement-filter, dedupe, BCF, or dueOf logic touched. VERIFY: sandbox transpile blocked by the F:-mount truncation (mount serves Reports.jsx cut at L574); host Read confirms file complete + edits balanced. Authoritative gate = host `tsc` in the bat + Vercel `next build` (SWC parses .jsx, fails deploy on any error). Re-check `/crm/marketing`+`/crm/reports` + US country score after ~24-48h fresh samples (they're low-traffic, so p75 stays noisy regardless).

PUBLIC-SITE MOBILE PERF — RENDER-BLOCKING PAGE TRANSITION KILLED (2026-07-05, CODE EDITED — push via RUN_PUSH_PERF.bat):
- SYMPTOM: Vercel Speed Insights, project hotelfountainbd-crm, MOBILE route `/` = Real Experience Score 37 (Poor): FCP 4.18s, LCP 4.58s, INP 29,664ms. CLS 0 and TTFB 1.21s already fine (left alone). Low sample count → p75 noisy (the 29.6s INP is likely one bad session amplified), so the fixes target the STRUCTURAL cause, not the number.
- ROOT CAUSE: `app/(site)/template.tsx` wrapped EVERY route in a framer-motion enter that started at `opacity:0` + `filter:blur(8px)`. Nothing painted until the framer-motion bundle downloaded + hydrated → inflated FCP/LCP (hero is the LCP element, hidden until JS), and animating a full-page `blur()` forced repeated whole-page rasterization → INP/TBT on mobile.
- FIX 1 (the big one) `app/(site)/template.tsx`: replaced the framer motion.div with a plain server-rendered `<div className="page-enter">`. New `.page-enter` CSS keyframe in `app/globals.css` (opacity + translateY(16px), 0.45s, NO blur) runs at first paint with zero JS, disabled under prefers-reduced-motion. Content paints immediately; framer no longer gates the critical path.
- FIX 2 `src/components/site/Parallax.tsx`: gated the scroll-linked motion value to desktop only (`window.matchMedia("(min-width:1024px) and (pointer:fine)")`, set post-mount → no hydration mismatch). On phones the parallax drift is imperceptible but the per-scroll-frame transform update was pure main-thread INP cost. `active = mounted && desktop && !reduce`.
- FIX 3 `next.config.mjs`: added `experimental.optimizePackageImports:['framer-motion']` (barrel tree-shake → smaller mobile bundle).
- FIX 4 `src/components/site/GlassCard.tsx` + `RoomCard.tsx`: removed framer-motion entirely. GlassCard is now a SERVER component; hover lift/glow via new `.glass-interactive` CSS (identical -6px offset + gold ring shadow + 0.35s ease). RoomCard keeps `"use client"` ONLY for the Book Now onClick; hover lift via `hover:-translate-y-2` + `motion-reduce` guard. ★ SAFETY: RoomCard's `variants={revealItem}` was DEAD CODE in both usages — the ScrollReveal stagger grid (home) and the AnimatePresence grid (RoomsCatalog) both pass explicit initial/animate OBJECTS, not variant LABELS, so framer never propagated "hidden"/"show" to the child → the variant never fired. Reveal/exit still owned by those parent wrappers, so no visual regression. GlassCard is used on all 5 public pages (home/rooms/services/contact/faq) so dropping framer from it shrinks the bundle site-wide. `revealItem` is now an unused export in ScrollReveal.tsx (harmless, left in place).
- VERIFY: all 6 files' syntax validated in isolation via ts.transpileModule in /tmp (sandbox tsc UNRELIABLE — F: mount served falsely-truncated views of template.tsx + Parallax.tsx, the documented [[write_tool_nul_padding]] / F:-mount phantom-truncation family). Authoritative `npx tsc --noEmit` runs inside RUN_PUSH_PERF.bat before commit.
- FILES: app/(site)/template.tsx, app/globals.css, src/components/site/Parallax.tsx, src/components/site/GlassCard.tsx, src/components/site/RoomCard.tsx, next.config.mjs (+ MEMORY_LOG.md). Push = RUN_PUSH_PERF.bat from Windows Explorer (typecheck-gated). Re-check Speed Insights mobile after ~24h of fresh samples.
...
- ENFORCEMENT (defense-in-depth): authoritative gate is the SERVER route `app/api/crm/reservation/route.ts` `update` action — `isAdminRole` derived from `staff.role`; rejects check-in change (403 "Only management can change the check-in date") and check-out shorten (403 "Check-out can only be extended…"). The SPA runs on the anon key so UI gating alone is bypassable — server is the real lock. UI (`src/components/ReservationEditModal.jsx`) mirrors it: Check-In `disabled` + locked style for non-admins, Check-Out `min={_origCheckOut}` + onChange clamp + "locked / extend only" hints + an early `save()` guard with the same messages.
- EMAIL: new server-only `src/lib/changeNotify.ts` — `notifyReservationChange()` (field diff: check_in/check_out/room_ids/status/paid/discount/notes/guest) + `notifyReservationDeleted()` (snapshot, since delete has no after-state). Sent via the existing Brevo transactional pipeline (`BREVO_API_KEY`, sender `HOTEL_SENDER_EMAIL`=hotellfountainbd@gmail.com) to `ADMIN_NOTIFY_EMAIL` (defaults to ahmedshanwaz5@gmail.com). Both calls are best-effort `.catch(()=>{})` — never block or fault the save/delete. Route now also selects `staff.name` (for the actor) and the delete branch selects the extra snapshot columns.
- "Gmail connector" intent satisfied via Brevo→owner-Gmail-inbox: the deployed Next.js route can't call a desktop Gmail tool; Brevo from the hotel Gmail address is the only server-side path. Subject names the actor; fires for admin edits too (owner's "anyone/anything" wording).
- ENV: `BREVO_API_KEY` already live in prod; optional `ADMIN_NOTIFY_EMAIL` override (defaults to owner). No DB migration, no schema change.
- VERIFY: sandbox `tsc --noEmit` clean for the 3 touched files; 0 NUL padding on all. Authoritative host-side `tsc` before commit (F: mount truncation artifact — see [[write_tool_nul_padding]] family).
- MINOR: notes-field first-save may show a one-time "— → text" diff if legacy text lives in `special_requests` not `notes` (harmless). Edit-email is now STAFF-ONLY (2026-07-01): the route wraps the notifyReservationChange call in `if (!isAdminRole)` (route.ts ~L235) so admin edits do NOT email. Delete-email still fires for every delete (deletes are always admin by RBAC, but a destructive cascade delete warrants the notice).

PUBLIC-SITE AI CHAT WIDGET EMBED — (site)-SCOPED SCRIPT + CSP ALLOWLIST (2026-06-29, ✅ CODE EDITED — NOT yet committed; owner typechecks + pushes from PowerShell):
- FEATURE (owner-requested): embed the AI front-desk chat widget into the PUBLIC marketing site only. Widget was demoed at `https://187-127-187-66.sslip.io/demo` (a raw-IP sslip.io host); `/demo` 200, `/chat` POST → 200 returning correct BDT rates (Fountain Deluxe ৳4,000 … Royal Suite ৳9,000), EN/Bangla, LLM-backed.
- WIDGET REVIEW (`/widget.js`, full source read via Chrome MCP): vanilla-JS IIFE, ZERO deps. ONLY network call = `POST {script-origin}/chat` with `{message, history(last 12)}`. Sends ONLY the visitor's typed chat text — does NOT read cookies / localStorage / host-page DOM / form fields; no analytics/fingerprinting/3rd-party calls; no eval/dynamic-script. Renders all messages via `textContent` (server reply can't XSS). Backend reply shape `{reply, lead}` (lead null until detected → server-side lead extraction). Verdict: current snapshot benign; standing risk is only continuous trust in the host (it can swap widget.js anytime).
- DECISION (owner-chosen): load from a FIRST-PARTY subdomain `chat.fountainbd.com` (owner controls origin + cert), NOT the raw-IP sslip.io host it was demoed on. No SRI hash (widget served live → a pin would break on every update); first-party origin is the mitigation instead.
- EDIT 1 `app/(site)/layout.tsx`: added module const `CHAT_WIDGET_SRC="https://chat.fountainbd.com/widget.js"` + `<script src={CHAT_WIDGET_SRC} nonce={nonce} defer />` placed just before `<ScrollProgress/>` INSIDE `SiteLayout`. Because it lives in the `(site)` layout it renders on public pages ONLY — never on `/crm` (different layout + perimeter gate). Carries the per-request `x-nonce` (matches the JSON-LD / FB-pixel pattern in the same file).
- EDIT 2 `middleware.ts` `buildCsp`: appended `https://chat.fountainbd.com` to BOTH `script-src` (L72) and `connect-src` (L76). REQUIRED because the CSP is nonce-based with NO `strict-dynamic` → an external script needs the host in `script-src`, and the widget's `fetch('/chat')` needs it in `connect-src`. Merge-only, existing entries untouched.
- VERIFY: files confirmed intact via host Read; 0 NUL padding (`grep -c -a -P '\x00'` = 0 on both); both CSP lines correct. ★ Sandbox `tsc` could NOT be trusted — the F: mount served a TRUNCATED view of `middleware.ts` (line 138 cut mid-string at `favicon.i` → false TS1002 on an UNCHANGED line); host Read shows L136–140 fully intact. Documented phantom-read artifact ([[write_tool_nul_padding]] family). Run `npx tsc --noEmit` from PowerShell as the authoritative gate before commit.
- OPEN (owner actions): (1) CONFIRM the assistant server is owner-controlled (raw-IP host still unverified); (2) point `chat.fountainbd.com` DNS at it + valid cert — the `<script>` tag is DORMANT/no-op until DNS resolves (widget simply won't appear, harmless); (3) host-side `tsc` then commit + push from PowerShell; (4) add a privacy-policy line (widget collects guest messages + extracts leads).
- ★ REVISED SAME SESSION → VERCEL SAME-ORIGIN PROXY (owner-chosen; SUPERSEDES the subdomain/DNS/cert plan in the bullets above). FINDINGS that forced the change: `fountainbd.com` DNS is on Vercel (`ns1/ns2.vercel-dns.com`); `chat.fountainbd.com` already WILDCARD-resolves to Vercel anycast (216.198.79.x), NOT the bot box; the bot box `187.127.187.66` has no cert for that name and I have no server access to issue one; the connected Vercel MCP exposes no DNS-record tool. So we went same-origin instead of a subdomain:
  - (a) `public/widget.js` = a PINNED copy of the widget we control (also eliminates the "host can silently swap widget.js" trust risk → no SRI needed).
  - (b) NEW `app/chat/route.ts` = POST proxy forwarding to `${CHAT_BOT_ORIGIN || 'https://187-127-187-66.sslip.io'}/chat` SERVER-SIDE (runtime nodejs, force-dynamic, 20s `AbortSignal.timeout`, mirrors the widget's own fallback message on error). Browser only ever talks to fountainbd.com.
  - (c) `app/(site)/layout.tsx` `CHAT_WIDGET_SRC` now `/widget.js` (relative → widget's `new URL(currentScript.src).origin` = fountainbd.com → `fetch('/chat')` same-origin).
  - (d) the `chat.fountainbd.com` tokens were REMOVED from the middleware CSP — `'self'` now covers BOTH the script load and the `/chat` POST. CSP back to its original form + nothing third-party.
  - SCOPE/SAFETY: NO DNS change, NO new cert. Middleware perimeter gate untouched (`/chat` + `/widget.js` are neither `/crm` nor `/api/crm`; static `/widget.js` and the proxied `/chat` just get harmless extra response headers). Bot-server ownership CONFIRMED by owner. All 4 touched files NUL-scanned clean (`app/(site)/layout.tsx` needed a 10-byte trailing-NUL strip after the Edit — the documented [[write_tool_nul_padding]] F: artifact).
  - OPEN (owner): host-side `npx tsc --noEmit` (sandbox tsc unreliable — F: mount serves a truncated view of middleware.ts) → commit + push from PowerShell; optional `CHAT_BOT_ORIGIN` env var if the bot host ever moves; add a privacy-policy line (widget collects guest messages + server-side lead extraction).
- ★★ SHIPPED + LIVE on prod (2026-06-29, fountainbd.com) — commit trail `8861271` (same-origin widget + /chat proxy: app/(site)/layout.tsx, app/chat/route.ts, public/widget.js) → `c47f0a4` (brand restyle) → `1e58d67` (next/script loader) → `d2a64dd` (cache-bust ?v=2). All Vercel READY. Verified via Chrome MCP: `/widget.js` 200 same-origin, `/chat` proxy 200 with real LLM reply, bubble mounts + opens, dark+gold styling, positioned above the Book Now FAB.
- ★ TWO PROD-ONLY GOTCHAS FIXED (both invisible to host Read / tsc — only show on the live hydrated page):
  - (1) RAW `<script>` WENT INERT AFTER HYDRATION: a `<script src defer>` rendered in the (site) layout JSX was present in the DOM but NEVER executed — a React #418 hydration mismatch made React re-insert the node programmatically, and browsers don't run a `<script>` re-created that way (so `#hf-btn` never mounted, intermittently). FIX: load via `next/script` `strategy="afterInteractive"` (commit 1e58d67). RULE: never load an external widget with a raw `<script>` in App-Router JSX — always `next/script`.
  - (2) `/public/widget.js` IS CACHED HARD: editing `public/widget.js` does NOT reach returning visitors (browser/CDN cache the static file → stale old widget served even after deploy). FIX/RULE: the script src carries a version query `"/widget.js?v=N"` in `app/(site)/layout.tsx`; **BUMP N every time `public/widget.js` changes** (currently `v=2`). The query string is the cache-bust.
- WIDGET BRAND TOKENS (`public/widget.js`): bubble bg `#0d1117` · gold icon `#c8a96e` · `1px rgba(200,169,110,.55)` border · `bottom:80px;right:20px` (clears the gold Book Now FAB which is `bottom:20px;right:20px;z-50`); window bg `#0f141b`, header `linear-gradient(120deg,#c8a96e,#e4cfa0)` with `#1a1407` text, msgs bg `#0b0f14`, user bubble gold-gradient/`#1a1407`, assistant `#161d26`/`#ece6da` + gold hairline, SEND gold-gradient. Font `'DM Sans'` (inherits the site's loaded face). Inline SVG chat-bubble icon (replaced the 💬 emoji).

DAILY REPORT DOWNLOAD — CLOSED-DAY PRINT PATH + SINGLE-A4 COMPACTION (2026-06-25, ✅ SHIPPED + LIVE — commits 92ce3f8 then 71a81af on main):
- BUG (owner reported): downloading the report on a CLOSED / previous day printed the raw web page (sidebar + mobile bottom tab bar), not the branded "Daily Performance Report". Open-day download was already correct.
- CAUSE: the `#print-report` block AND its `@media print` <style> lived ONLY in the LIVE (open-day) return of `Daily` in `src/components/Reports.jsx`. The post-close (`if (closeRow)`) branch returns earlier with its own "⬇ Download → window.print()" button but NO print DOM and NO print stylesheet → window.print() fell back to printing the screen chrome.
- FIX 1 (commit 92ce3f8): extracted the print CSS to a module-scope `const PRINT_CSS`; rendered a closed-day `#print-report` mapped to the locked snapshot (Closed Collection + New-after-close + Total; New Movements; New Collections; Outstanding Dues + total row) plus `<style>{PRINT_CSS}</style>`; live badge made dynamic `onOpenDay ? 'LIVE — OPEN DAY' : 'HISTORICAL'`. The two `#print-report` blocks never coexist (mutually-exclusive returns), so the shared `body *{visibility:hidden}` has exactly one visible target — no duplicate-id conflict.
- FIX 2 (commit 71a81af): report spilled a lone footer onto a 2nd A4 sheet. Compacted the shared PRINT_CSS: @page margin 0.7in→7mm 9mm (reclaims ~22mm vertical), base 8.5/1.18→8/1.12, panel margin-bottom 11→6px, footer margin-top 7→4px, td padding 1.5→1px, and removed `.pr-tbl`/`.pr-dues` from the whole-block `page-break-inside:avoid` group so big tables break row-by-row instead of jumping whole.
- SCOPE/HYGIENE: both pushes were path-scoped `git commit -- src/components/Reports.jsx` — left the in-progress Meta CAPI work (META_CAPI_SETUP.md, src/lib/capi.ts, scripts/capi_aplus_audit.mjs, supabase migration, route edits, crm.html) staged + uncommitted for owner. Babel parse-clean; pre-commit hooks (createRoot/crm.html/EOF) passed both times.
- NOTE: closed-day figures intentionally reflect the locked snapshot, not open-day totals — only the print TEMPLATE is identical to the open-day report.
- STALE-LOCK: a Windows-owned `.git/index.lock` blocked the first commit; cleared via PowerShell `Remove-Item .git\index.lock -Force` (sandbox cannot unlink it — "Operation not permitted"). Re-confirms the no-sandbox-git rule.
- FOLLOW-UP A (commit f20f9f9): owner clarified the closed-day download must be the FULL day-closing DETAIL report (FINANCIAL + PAYMENT METHOD + that day's Daily Movements + Outstanding), NOT the post-close "new activity" view (which was showing next-day movements dated after the closed day). FIX: refactored the print markup into ONE shared `renderPrint(badgeText, o={})` used by both branches; the closed branch passes the day's own computed figures (collected/moves/paySplit/allDue) — when viewing a non-open day `inDayRange` is `d===date`, so they already reflect that single day. Single A4 retained.
- FOLLOW-UP B (commit 9eb5cbe): Opening Token & Payouts were not persisted (derived at render, lost after close → re-downloads showed ৳0 / Closing=Total). FIX: added `opening_token` + `payouts` numeric DEFAULT 0 columns to `night_audit_log` (migration 20260625_night_audit_token_payouts.sql, applied live; additive, RLS unchanged, advisor sweep = no NEW findings). `close-day/route.ts` now reads them from the POST body and writes them via a post-RPC `UPDATE` (the canonical `execute_nightly_audit` RPC is NOT modified — it owns every computed column). `handleClose` sends `opening_token:tok, payouts:payout`; Reports `fetchData` selects them back; `renderPrint` takes `o.tok/o.payout/o.closing` overrides and the closed branch passes `closeRow.opening_token/payouts` with `closing = token + collected − payouts`.
- LIMITATION: days closed BEFORE 9eb5cbe deploy have opening_token=payouts=0 (never captured) → their re-downloads still show ৳0 for those two lines + Closing=Total Collection. Only closes going forward are exact. Backfill possible per-day via direct UPDATE if owner supplies the figures.
- FOLLOW-UP C (token/payout hardening + UI, this session): owner rule — "each day's payout/token MUST save to that specific day's report". (1) `tok`/`payout` now FALL BACK to the saved `closeRow.opening_token`/`closeRow.payouts` when the inputs are blank → re-closing a day no longer ZEROES them (the closed view had no inputs, so a Re-close was sending 0/0). (2) Added Opening Token + Payouts inputs to the CLOSED-day action bar (placeholder shows the saved value) so any PAST day's drawer figures can be set/corrected from the UI and saved via Re-close — no SQL needed anymore. (3) closed-day `renderPrint` reverted to defaults since `tok`/`payout` already resolve to the snapshot. BACKFILL: 24-Jun-2026 row was payouts=0 (closed before the feature) → set `payouts=21385` via direct UPDATE (opening_token stays 0) ⇒ Closing = 57,000 − 21,385 = ৳35,615, matching the owner's pre-close screenshot.
- ★ TOTAL-COLLECTION SOURCE on a closed day's PRINT: uses the RECOMPUTED `collected` (Σ txs with fiscal_day===date) = ৳57,000 for 24-Jun, NOT the locked `night_audit_log.total_collections` = ৳73,000. They differ because the locked snapshot captured collections through the close moment (spanning into the next calendar day) while the recomputed figure is fiscal_day-scoped. Owner's reference report shows ৳57,000, so the recomputed value is intentionally kept; do NOT switch the print to `total_collections` without owner re-approval.
- FOLLOW-UP D (Monthly/Yearly bars, this session): both charts rendered BLANK while totals were correct (Month ৳1,025,705 · Year ৳2,703,626). CAUSE: `RevBars` bar height was a PERCENT (`${(v/max)*100}%`) resolved against a flex column with NO definite height → collapsed to 0 (only axis labels + baseline showed). FIX: size bars in PIXELS — `height: v>0 ? Math.max(2, Math.round((v/max)*160)) : 0`, row height 188px, rounded top corners. `--iv-side` (#0D1117) was a red herring (dark = visible); the bug was purely the indefinite-parent percent-height collapse. If any future flex-child uses `%` height, give the parent an explicit px height or size the child in px.
- SESSION COMMIT TRAIL (all path-scoped to Reports.jsx unless noted; CAPI WIP deliberately left uncommitted throughout): 92ce3f8 (closed-day print path) → 71a81af (single-A4 compaction) → f20f9f9 (full day-detail report) → 9eb5cbe (token/payouts persistence: Reports.jsx + app/api/crm/close-day/route.ts + migration 20260625_night_audit_token_payouts.sql) → [token/payout UI hardening] → [Monthly/Yearly bars]. DB: ALTER night_audit_log ADD opening_token, payouts (applied live).
- FOLLOW-UP E (Monthly/Yearly UI + downloadable, this session): owner wanted the charts to look professional and the Monthly/Yearly revenue reports to be downloadable. UI: rewrote `RevBars` — dashed horizontal gridlines with a left ৳ Y-axis scale (revK abbreviation, e.g. ৳12.3k), gold gradient bars (#C8A96E→#8B6914) with rounded tops, the PEAK period auto-highlighted brighter (#EBD3A0→#A9801F), hover brighten, and value labels above bars on the Yearly view (showValues; Monthly omits them — too many days). Styles live in module-level `REV_CSS`; bar heights stay PX (the FOLLOW-UP D fix). DOWNLOAD: new module-level `RevReportPrint({kind,periodLabel,rows,total})` renders the `#print-report` DOM reusing PRINT_CSS — Summary card (Total / Active periods / Avg per active), Peak card, Scope card, and a Breakdown table (Day|Month · Revenue · Share%) with a Total row. Monthly/Yearly each got a ⬇ Download button (window.print()) alongside the period picker, and render `<RevReportPrint/>`. Safe because Reports() mounts only ONE tab at a time, so only one `#print-report` exists in the DOM (Daily's renderPrint vs RevReportPrint never coexist).


DAY-CLOSE "CLOSING BALANCE" FORMULA FIX — CLOSING = TOKEN + FULL-COLLECTION − PAYOUTS (2026-06-24, ✅ SHIPPED + LIVE — commits fa9412b THEN 4896ef9 on main, Vercel prod READY):
- ★★ FINAL FORMULA (commit 4896ef9, supersedes fa9412b): Closing Balance = `Opening Token + Cash Collection − Payouts`, where **Cash Collection = the FULL day's collection = sum of ALL payment methods (cash + bKash + card + …) = Total Collection**. Owner spec 2026-06-24: do NOT exclude digital methods. Code: `const cashIn = collected; const closing = tok + cashIn - payout;` (`src/components/Reports.jsx`). Live example (06-Jun open day): Total ৳39,700 · Cash Collection ৳39,700 · Payouts ৳8,370 · Opening Token ৳0 → Closing **৳31,330** (was wrongly ৳23,330 under the digital-exclusion version). ⇒ The "DIGITAL regex / digital-exclusion" and the "OPEN CAVEAT (cash-tagging)" bullets below (from fa9412b) are SUPERSEDED + MOOT.
- ⚠ F:-MOUNT NUL PADDING during this edit ([[write_tool_nul_padding]]): host Read showed Reports.jsx ending cleanly at `}` but the file had 280 trailing 0x00 bytes appended by the Edit → babel parse failed "Unexpected character" at EOF. Stripped via sandbox `python rstrip(b'\x00')` (bash CAN write the mount; only `rm` needs allow_cowork_file_delete). Re-parse clean, THEN commit. Always NUL-scan (`grep -c -a -P '\x00'`) Reports.jsx after an Edit before pushing.
- BUG (owner reported "CRM day closing showing wrong"): Reports → Daily "Closing Balance" was computed as `collected − tok` (`src/components/Reports.jsx` ~L140). On the 23-Jun report that produced ৳29,115 = Total Collection ৳42,500 − Opening Token ৳13,385 — i.e. it SUBTRACTED the opening float from total collection. Backwards for a cash drawer (and can go NEGATIVE when token > collection).
- DECISION (permanent, owner-confirmed option A): Closing Balance = the physical cash drawer to hand to the next shift = `Opening Token (float) + Cash Collected − Payouts`. Opening Token = yesterday's closing float (manual input); Payouts = cash paid out (new manual input, default 0).
- ★ CASH DEFINITION: cash = the day's collection MINUS explicitly-digital methods (`/bkash|nagad|card|bank|account|transfer/i` on tx `type`). Untagged types (e.g. "Stay Extension (+21 nights)") DEFAULT TO CASH on this cash-first property. This reconciles EXACTLY to the report's Payment-Method "Cash" line (23-Jun: 42,500 − 5,500 bKash = ৳37,000). Do NOT switch to a `/cash/i`-only match — types like Stay Extension carry no method tag and would be wrongly dropped from the drawer (undercount).
- DB IS CORRECT — this was a REPORT-LAYER bug only. Verified: `transactions` for `fiscal_day='2026-06-17'` sum to exactly ৳125,500 = `night_audit_log.total_collections` = the on-screen "Closed Collection". "Opening Token"/"Closing Balance" are NOT stored anywhere (no column in night_audit_log, no cash/float table) — they are derived in Reports.jsx at render time.
- IMPL (`src/components/Reports.jsx`): added `payouts` state + "Payouts ৳" input beside the Opening Token input (both reset on close); `const closing = tok + cashIn - payout` where `cashIn = Σ non-digital txns for the day`; relabeled the Closing Ledger card + A4 print "Financial" block to the new breakdown (Opening Token → + Cash Collected → − Payouts → Closing Balance); StatCard sub "collection − token" → "token + cash − payouts". Display/calc only — no money write-path, dedupe, or movement logic touched.
- VERIFY (real data): 23-Jun corrected closing = 13,385 + 37,000 − 0 = ৳50,385 (was ৳29,115); 17-Jun cash_in = 125,500 (all cash, 0 digital). Babel parse-clean; pre-commit hooks (createRoot/crm.html/EOF) passed; Vercel built READY.
- ★ LIVE FRONT-END = the Next app `src/components/Reports.jsx` (imported by `app/crm/reports/page.tsx`). The standalone `public/crm-src.jsx`/`crm-bundle.js` carry the SAME old `collected - tok` but are NOT served — do NOT chase them unless the bundle is ever re-served.
- OPEN CAVEAT (cash-tagging): the Payment-Method "Cash" card uses a `/cash/i` tag match, so an untagged cash payment (Stay Extension etc.) shows under "Other" there while the drawer "Cash Collected" correctly counts it as cash → the two cash figures can differ on a day with untagged types (17-Jun: method-card Cash ৳31,000 vs drawer cash ৳125,500). Real fix = stamp a payment method on every transaction type. Deferred — owner to decide whether to reconcile the method card to the same digital-exclusion logic.
- COMMIT HYGIENE: pushed ONLY Reports.jsx (RUN_PUSH_CLOSING_FIX.bat did `git reset` then `git add` the one file); the pre-existing uncommitted `public/crm.html` working-tree change was deliberately LEFT OUT (still uncommitted for owner to handle).

REPORTS DAILY MOVEMENTS — "DUE OUT" vs "CHECK-OUT" LABEL RULE (2026-06-19, ✅ SHIPPED + LIVE — commit 3e286ae, Vercel prod deploy READY):
- DECISION (permanent, owner-approved): in `src/components/Reports.jsx` `Daily`, a movement row built from a reservation whose check_out DATE is in the open-day range is only labeled **"Check-Out"** once the guest is ACTUALLY departed (`upper(status)==='CHECKED_OUT'`). Until then it is a pending **"Due Out"** (amber badge). A blank Time ("—") on a Due Out row is CORRECT — no checkout has happened, so `checked_out_at` is legitimately NULL. When staff process the checkout, `trg_stamp_movement_times` stamps `now()` and the row flips to "Check-Out" + live time automatically.
- WHY: owner reported "some Time not showing" on Check-Out rows. ROOT CAUSE was NOT data loss / trigger failure — the trigger works (all 4 real checkouts on 06-19 had exact times, 0 null). The "—" rows (tuli Debnatha 407, SI SHAMIM 510, MOHAMMED RUBEL 505) were all still `CHECKED_IN`, due out today but not yet checked out. Reports built an "OUT" row from check_out DATE (line ~103 `outs = res.filter(check_out in range)`) regardless of status, so a not-yet-departed guest showed as "Check-Out — —". Relabel removes the confusion.
- IMPL: helpers `isDeparted(m)` / `typeLabel(m)` / `typeTone(m)` after `timeOf` (~L122); applied to all 3 render sites (post-close New Movements table, open-day desktop table, mobile/print card). Display-only — no money math, dedupe (`_movesColl`/`_keep`), or time logic touched. tone: Check-Out=teal, Due Out=amber.
- PRE-TRIGGER HISTORY GAP (unchanged): checkouts ≤2026-06-17 have `checked_out_at`=NULL (the 06-18 backfill populated checked_in_at from created_at but left checkouts blank — unrecoverable). They show "—" only on PAST-day reports. Exact time unrecoverable; only approximable from last-tx created_at. NOT backfilled (owner-approval pending; approximate-only).
- ★ F:-MOUNT PHANTOM during this fix: sandbox `tsc`/`babel`/`git diff`/`wc` all reported Reports.jsx truncated at line 419 ("'}' expected", "No newline at EOF"). FALSE — host Read API showed the file fully intact (419 orig + 8 inserted lines, balanced, ends at Yearly's `}`), HEAD blob (`git show`, reads .git object store not the mount) confirmed the original legitimately ends at line 419, and Vercel built 3e286ae to READY. Lesson re-confirmed: trust host Read + `git show <blob>` + Vercel build over the Linux mount's byte-view; the mount serves stale/truncated phantoms for F: files.

CRM PERIMETER ACCESS GATE — OFFICE-IP + ADMIN-REMOTE (2026-06-19, ✅ SHIPPED + VERIFIED LIVE — commit afb5e91 pushed to origin/main, Vercel prod deploy READY; admin-remote + office-IP access confirmed live):
- STATUS: LIVE on prod. Verified: (1) office PC on 103.113.153.228 → /crm loads authenticated dashboard, no block; (2) phone on cellular logged in as ADMIN → CRM works; (3) off-network protected /api/crm returns no data (gated); (4) public site (fountainbd.com/) open off-network. Remaining optional: cellular as receptionist → blocked (same code path as the verified admin check, only role not in REMOTE_ROLES → denies).
- FEATURE (owner-requested "make CRM accessible only to selective desktops"): restrict /crm + /api/crm at the edge. POLICY: on the hotel office WAN IP → any logged-in staff; OFF-network → ADMIN-tier session only; everyone else 403 (API) / redirect to /crm login (pages). Public site (/, /api/book, /api/invoice/*) untouched.
- ★ KEY DECISION — GATE IN middleware.ts, NOT the Vercel Firewall (A1 plan REJECTED): the Firewall is IP+path only and ROLE-BLIND, so it cannot express "admin allowed from anywhere, others office-only." A hard Firewall Deny would also block an off-site admin's request BEFORE middleware runs. Therefore do NOT add the "Deny CRM elsewhere" firewall rule — middleware does all gating. (An optional Firewall *Allow office* fast-path is harmless but unnecessary.)
- ★ EDGE-RUNTIME HMAC RE-VERIFY: the lumea_sess session cookie is the CUSTOM HMAC token from src/lib/session.ts (base64url(json).sig), NOT a JWT. Middleware runs on the EDGE runtime and CANNOT import session.ts (it uses Node `crypto`), so the gate re-verifies the same HMAC-SHA256 with Web Crypto (crypto.subtle, importKey raw + sign, base64url-compared) and checks the 7-day iat window. Secret = SESSION_SECRET (must stay set in Vercel all envs, or every off-site verify fails). jose/jsonwebtoken NOT used — zero new deps.
- ★ DENY-ONLY SHORT-CIRCUIT: the gate is prepended to the EXISTING middleware (tenant-slug + per-request CSP-nonce). It returns early ONLY on deny; ALLOWED requests fall through untouched so CSP/tenant headers the app depends on still apply. AUTH_EXEMPT = {/api/crm/login, /api/crm/session} always pass so an off-site admin can sign in / refresh. Bare /crm always renders (login shell); deeper pages redirect to it; data stays protected by the /api/crm 403.
- KNOBS (top of middleware.ts): `OFFICE_IPS = ['103.113.153.228']` (hotel public WAN IP — confirmed STATIC via ISP + api.ipify.org; add a 2nd entry per extra ISP line). `REMOTE_ROLES = {owner, admin}` — STRICT per "only admin"; manager/receptionist/housekeeping are office-only (add 'manager' to widen). Mirrors permissions.js isAdmin() intent but narrower.
- PERIMETER ONLY — Supabase RLS + the anon-key revoke ([[c3_anon_read_lockdown]]) remain the real data guard.
- VERIFY: middleware.ts tsc-clean (0 errors in file) + 0 NUL bytes. The 35 repo-wide tsc errors are pre-existing F:-mount NUL-read phantoms in untouched files (login/route.ts, layout.tsx) — confirm clean from PowerShell before push.

DAILY MOVEMENTS "TIME" COLUMN — TRIGGER-CAPTURED ACTION TIMESTAMPS (2026-06-18, DB live on prod; UI commit 6edffe8 pushed + LIVE):
- FEATURE (owner-requested): Reports → Daily Movements now shows the live time each check-in/out actually happened, sorted chronologically.
- ★ KEY DECISION — CAPTURE VIA DB TRIGGER, NOT SERVER ROUTE: status flips to CHECKED_IN/CHECKED_OUT happen on MULTIPLE write paths (server `/api/crm/reservation` AND client anon writes from CheckActionModal/Rooms/etc.), so a route-level timestamp would miss the client paths. A `BEFORE INSERT OR UPDATE` trigger on `reservations` is the only single capture point. This is a permanent architecture decision.
- DB (APPLIED to prod `mynwfkg`, migration `add_movement_action_timestamps`): added `reservations.checked_in_at` + `checked_out_at` (timestamptz, nullable) — DISTINCT from the calendar `check_in`/`check_out` (stay dates at 00:00:00; `check_in_time`/`check_out_time` are NULL/dropped). Fn `trg_stamp_movement_times()` (SECURITY INVOKER, search_path=public) stamps `now()` on the TRANSITION into CHECKED_IN/CHECKED_OUT, keyed on `upper(status)` (fire-order-robust vs the status-uppercase trigger) with `COALESCE` so an existing stamp is NEVER overwritten (idempotent — re-saving a folio/recalc won't reset the time). Anon needs no column UPDATE grant (trigger sets NEW server-side). Trigger `trg_stamp_movement_times BEFORE INSERT OR UPDATE`.
- BACKFILL (owner-approved): 1,249 past CHECKED_IN/OUT rows set `checked_in_at = created_at` (rough proxy = booking time); checkouts left blank (unrecoverable), populate forward. Verified live via a transactional status-flip + ROLLBACK (stamped out_at=now, preserved in_at) — prod data untouched.
- UI (`src/components/Reports.jsx`, commit 6edffe8): reservations `.select` +`checked_in_at, checked_out_at`; `timeOf(m)` = IN→checked_in_at / OUT→checked_out_at / PAY→latest owning tx `created_at` today; new "Time" column after Type in the live Daily Movements table (HH:MM AM/PM Asia/Dhaka via existing `fmtTime`), colSpans 6→7; `moves`/`moveColl` now sorted chronologically by `timeOf` (nulls last) via an index map that PRESERVES the dedupe-before-filter ghost-row rule (reorders kept rows only — `collected`/`moves.length` unchanged, no double-count). A4 print report (`#print-report` lives IN Reports.jsx, NOT legacy crm-src.jsx): time STACKED under the Type label to keep the single-page 6-col width. Post-close "New Movements" table left as-is (shows future scheduled Date, not action Time). tsc clean.

GUEST_ID PHANTOM COLUMN FIX — reservations.guest_id → guest_ids (2026-06-18, code fix commit 96feb84 pushed + LIVE; edge fn v14 live):
- TRIGGER: nightly CRM Orchestra audit found a recurring postgres ERROR `column reservations.guest_id does not exist` (2×/24h). Root cause: two code paths read a scalar `guest_id` on `reservations`, which only has `guest_ids uuid[]` (canonical). ALL DB functions/views were already correct (use `guest_ids`) — the bug was code-side only.
- FIX 1 (`supabase/functions/lighthouse-summary/index.ts:85`): `.select('... vip, guest_id, total_amount')` → `guest_ids` (the value was selected-but-unused; the non-existent column is what threw). REDEPLOYED via Supabase MCP → edge fn **version 14, ACTIVE, verify_jwt=false** (function uses custom `Bearer CRON_SECRET` auth — deploying with the tool default `verify_jwt=true` would 401 the pg_cron invocation). LIVE on prod `mynwfkg`.
- FIX 2 (`app/api/ai/assist/route.ts:107`): reservations filter `guest_id=eq.${scope.guest_id}` → `guest_ids=cs.{${scope.guest_id}}` (PostgREST array-contains). `scope.guest_id` (input param) and the `guests` table query at line 101 are CORRECT and untouched. Vercel route — shipped by the push (deploys on Vercel).
- ★ PUSH DRIFT: the first plain push was rejected non-fast-forward — local was based on 49973b0 but origin had advanced 2 commits (demand-forecast 4b20640+5b6b048, pushed from another env). Resolved by a CODE-ONLY rebase (`RUN_PUSH_GUESTID_FIX_v3.bat`): dropped MEMORY_LOG.md from the commit to avoid a top-of-file conflict, rebased the 2 code files onto origin (clean), pushed `5b6b048..96feb84`. Demand-forecast commits preserved (NO force-push). This MEMORY_LOG entry re-added in the follow-up.
- VERIFIED: both query shapes resolve read-only on prod (lighthouse select returns `guest_ids`; `guest_ids @> array[uuid]` works). Eliminates the nightly 19:00 UTC Lighthouse cron error.
- NOT TOUCHED (owner-deferred P3): legacy duplicate ৳100 Advance Payment (tx `c48b91d8`, res `f1a0db6a`, 2026-05-20, pre-idempotency-key era).

DEMAND-FORECAST ENGINE + DEMAND-AWARE LEAD PROSPECTING (2026-06-17, commit 4b20640 pushed + LIVE; edge fns deployed live):
- FEATURE: a deterministic occupancy demand forecast now (a) powers the 9 AM AI lead engine and (b) shows as a new CRM "Demand Forecast" tab. Owner-requested "connect [morning leads] with Hotel demand forecast feature."
- SHARED ALGORITHM (`supabase/functions/_shared/forecast.ts`, mirrored as a sibling `forecast.ts` in each function dir for Deno bundling; JS twin lives inline in `public/crm-src.jsx` as `crmForecast`): for each of the next N days (default 14) counts DISTINCT room_ids occupied by reservations where status in {RESERVED,CHECKED_IN,CONFIRMED,PENDING} and check_in <= day < check_out, vs the LIVE rooms count. Outputs per-day occPct + demand tier (HIGH>=75 / MED>=45 / LOW), avgOccPct, roomNightsFree, weekdayGapPct, weekendGapPct (BD weekend = Fri/Sat). `planProspecting()` maps the gap -> emphasis segments (weekday-soft -> corporate/embassy/airlines; weekend-soft -> events/travel) + leads-per-segment (LOW=4, MED=3, HIGH=2) + a `demandContext` string injected into the Gemini prompt.
- ★ OCCUPANCY RULE (the key correctness invariant): a stay occupies nights [check_in, check_out) — the CHECKOUT DAY IS FREE. check_in/check_out are timestamptz, so compare on the DATE portion only (`String(v).slice(0,10)`). The first cut compared the raw timestamp string against a bare YYYY-MM-DD date; the trailing "T00:00:00+00:00" made `check_out > date` TRUE on the checkout day -> over-counted by 1 room per checkout (gave 374 vs the correct 376 room-nights free). Verified all 4 surfaces (SQL baseline, wf-demand-forecast, wf-seo-lead-morning ?dry=1, CRM crmForecast) now agree at 376.
- EDGE FUNCTIONS (deployed via Supabase MCP, project mynwfkg): NEW `wf-demand-forecast` (v3, verify_jwt=false, read-only, returns {forecast,plan}; `POST .../functions/v1/wf-demand-forecast?days=14`). UPDATED `wf-seo-lead-morning` (v44): computes forecast first, injects demandContext + scales perCategory + adds a forecast block to the brief email; PRESERVES the live dedup guard (`workflow_should_run`), in-flight lock (acquire/release), and vault Resend key EXACTLY. Added `?dry=1` (computes forecast+plan, NO writes, bypasses dedup) for safe testing.
- ★ REPO/PROD DRIFT FOUND: the repo's `supabase/functions/wf-seo-lead-morning/index.ts` was STALE (missing the live dedup guard/lock/vault that prod v42 had). Always `get_edge_function` the LIVE source before editing — editing the repo copy and redeploying would have wiped the guard. Repo is now back in sync with prod.
- CRM TAB: `ForecastPage` in `public/crm-src.jsx` computes client-side from already-loaded `data.reservations`+`data.rooms` (ZERO new DB calls, no RLS surface change). Added to NAV_ITEMS (Analytics section), render switch (`cur==='forecast'`), PAGE_TITLES, and owner/manager `pages` allowlists. Warm-Ivory styling reusing `.stat`/`.card` classes; 14/7/30-day bar chart + daily breakdown + "AI Lead Prospecting Focus" mirroring the engine.
- ★ crm.html WAS NUL-CORRUPTED: `public/crm.html` was 196,790 bytes of which 187,211 were trailing NUL padding (real HTML ended at the `</html>` at byte 9,571) — an OneDrive truncation artifact. Browsers ignore trailing NULs so prod still rendered. Rebuilt clean (~9.5 KB). NOTE: the rebase later showed the REMOTE crm.html was already de-corrupted but LACKED local mobile CSS (100dvh/overscroll) — resolved by keeping the local superset + bumped ?v.
- ★ BUILD PIPELINE GOTCHA: rebuilt the bundle with babel set up in /tmp/babel-tools. MUST use the project's classic JSX runtime (`{"runtime":"classic"}`) — passing the bare `@babel/preset-react` defaults to the AUTOMATIC runtime which injects an ESM `import {jsxDEV}` and breaks the global-React bundle. Pipeline: babel (classic) -> terser --compress --mangle -> public/crm-bundle.js, then bump `crm-bundle.js?v=` in crm.html. Edit crm-src.jsx via Python string-replace only (Edit tool truncates at the OneDrive mount).
- ★ PRE-EXISTING JSX BREAKAGE FIXED: crm-src.jsx hadn't compiled since before this work — the notification dropdown's "HK urgent tasks" block was missing its `{hkUrgent>0&&(` opener but still had the closing `)}` (present in git history too). Restored the opener to match the sibling "Dirty rooms" block so the bundle builds.
...
- VERIFICATION: 0 NUL bytes; confirmation body re-read on disk (Guest Details before grid, boxes removed, table intact). Owner runs `npm run typecheck` from PowerShell before push.
- PRINT FIT (follow-up same day): the added Guest Details cards pushed the confirmation onto a 2nd A4 sheet. Compacted PRINT-ONLY (`@media print`): GD_CSS gained a print block shrinking `.gd-*` (card padding 14/18→8/14, name 14→12px, value 12.5→10.5px, tighter gaps/margins) — applies to invoice too, harmless; confirmation's own print block now also compacts `.box` padding 18/20→9/14, `.grid`/`table`/`.totals` spacing, `.terms` margin, `.val` 13px (all `!important`). Screen layout unchanged. `.page` keeps `min-height:calc(297mm-16mm)` flex + `.ftr{margin-top:auto}` so the footer still pins to the page bottom. Fits 1 page for 1–2 guests; 3+ guests may still spill (acceptable).

PER-GUEST DETAIL BLOCK ON CONFIRMATION + INVOICE (2026-06-17, commit 8a8c408 pushed + LIVE on prod):
- DECISION: both printed docs now render a "Guest Details" section listing EACH guest separately with Full Name, ID Type, ID Number, Nationality, and Address — beyond the joined-names summary line added earlier same day. Owner-requested.
- SHARED HELPERS (`src/lib/printDocs.js`): `resolveGuests(res, guests)` maps `res.guest_ids` → full guest objects (order preserved). `guestDetailsHTML(guestObjs)` builds one `.gd-card` per guest (returns '' when no full objects, so docs degrade gracefully to the names summary). `GD_CSS` is the shared style block, injected into BOTH the confirmation and invoice `<style>` via `${GD_CSS}` and the section via `${guestDetailsHTML(guestObjs)}` (confirmation: after the box grid, before the room table; invoice: after the Billed-To/Status grid, before the charges table). Field mapping: Address = `[address, city, country].filter(Boolean).join(', ')`; ID Number = `id_number || id_card` (legacy combined col fallback); Nationality = `nationality`; missing → '—'.
- CALLER SELECTS WIDENED (guests table has these cols, verified via Supabase): `Reservations.jsx` (feeds ReservationEditModal→printConfirmation), `Rooms.jsx` (feeds RoomFolioModal→printInvoice), and `Billing.jsx` invoice on-click fetch all now `select('id, name, id_type, id_number, nationality, address, city, country, id_card')` instead of `id, name`. Reservations/Rooms snap caches will carry the wider shape on next warm; stale warm caches just show '—' until the live refetch.
- VERIFICATION: 0 NUL bytes on all 4 files; both `${guestDetailsHTML(guestObjs)}` injections confirmed via host Grep (confirmation + invoice). Sandbox tsc unreliable on this mount (frozen-view phantom errors) — owner runs `npm run typecheck` from PowerShell before push.

MULTI-GUEST ON PRINTED CONFIRMATION + INVOICE (2026-06-17, commit 51e6881 pushed + LIVE on prod):
...
- SERVER (`app/api/crm/folio/route.ts`): `create` now looks up the staff name by `sess.id` (service role) and stores `added_by_id`/`added_by_name`. `delete` now rejects non-admins with 403 via `isAdmin(sess.role)` — role comes from the signed HttpOnly session, NOT the client. This is the REAL enforcement point: anon has only SELECT on `folios` (no INSERT/UPDATE/DELETE), so the service-role route is the sole write path and the 403 cannot be bypassed with the anon key.
- UI (`RoomFolioModal.jsx` + `ReservationEditModal.jsx`): both call `useAuth()` + `isAdmin(user?.role)`; the delete (×) renders only for admins. Room charge and manual charges are now separate sections. ReservationEditModal was upgraded from summing extras to listing each charge row (id/amount/category/description/added_by_name/created_at) with attribution + gated delete; `resFolioExtras` is now a computed reduce over `chargeRows` (feeds `computedTotal`). Both modals bump a `reload` counter on add/delete so the list refreshes live.
- VERIFICATION CAVEAT (recurring): the Cowork bash sandbox served a STALE/frozen view of this F: mount — `tsc` errored on files NOT touched this session (`send-otp/route.ts`, `reservation/route.ts`). All edits verified via the Read tool instead (complete braces, tag-balanced JSX, 0 NUL bytes). Owner must run `npm run typecheck` from PowerShell before the push, then commit/deploy as usual.

CUSTOM LUMEA SKILLS + ORCHESTRATION-GATE WIRING (2026-06-14, commit 4725e83 pushed):
- Built + installed 5 Cowork skills tailored to this stack (visible as `anthropic-skills:lumea-*`): `lumea-systematic-debugging` (Shadow Audit before any financial fix + the ৳13,600 orphan rule + reservation_id anchor), `lumea-security` (RLS-always-on, SECURITY INVOKER views, least-privilege grants, tenant isolation, backup-first migs), `lumea-uiux-promax` (Warm Ivory tokens: Libre Baskerville/DM Sans/IBM Plex Mono, 1px #EAE6DD hairline, 2rem pad, Two-Gold rule #8B6914 text / #C8A96E fills-only, Subtle-Glow badges, cubic-bezier(0.4,0,0.2,1)), `lumea-seo-aeo` (public-site fountainbd.com metadata + Hotel/HotelRoom JSON-LD in BDT + single-H1 + keep /crm,/billing noindex), `lumea-playwright-e2e` (money-flow E2E + multi-room-checkout sibling-integrity + role/testid selectors, never waitForTimeout). Source + .skill files in Cowork outputs `lumea-skills/`.
- WIRED into the `.claude/commands/` orchestration gates (commit 4725e83): `/investigate`→lumea-systematic-debugging, `/review`→lumea-security + lumea-uiux-promax (+ lumea-seo-aeo for `app/(site)/**` diffs), `/qa`→lumea-playwright-e2e, `/ship`→lumea-seo-aeo (public-site pre-deploy). Each gate now names its canonical skill instead of duplicating the checklist inline. Gate inherited from MASTER_ORCHESTRATION.md: P3 auto-apply after QA GREEN, P1/P2 HOLD.
- "firmware" from the original skill request was NOT built — no fit for a Next.js/Supabase CRM; pending owner clarification.
- GIT HYGIENE NOTE: while committing the gate edits, the index was found corrupted (NTFS index-lock null-sha1) with spurious staged deletions of `vercel.json` + `vitest.config.ts`. Repaired by rebuilding `.git/index` from HEAD (working tree untouched); both files confirmed intact. The Cowork bash sandbox kept serving the STALE pre-repair index view afterward — trust the Windows git report, not sandbox `git status`, for this mount.
...
CASH DUP AUDIT (2026-06-13): Re-derived same-reservation/same-amount cash pairs. 11 pairs found. 4 already `[VOID-DUP]` (safe). 2 GENUINE multi-installment (cash_sum == net bill): MOHAMMAD RAIHAN 308 (4000×2=8000=bill), SOHAN 505 (3500×2=7000=bill). 5 SUSPECT — cash collected EXCEEDS the net room bill (phantom/dup or unbooked folio): ZUBAYED KHAN 306 (4000×2 vs net 3500), SI SHAMIM 510 (2000×2 vs net 2000), MD JUBAYER AHMED RAKIB 301 (3500×2 vs bill 3500), MD SOLIMAN TUHIN 505 (3500×2, 2nd tx named SARFARAJ MIA), ENJAMUL/res-now-SAJJADUL 505 (6000×2 vs reservation total 2000 — misattributed). `paid_amount` reflects only the bill (due math OK per PAID-AMOUNT POLICY); excess sits in the tx ledger and can inflate that fiscal day's Total Collection.
- RESOLVED 2026-06-13: 5 duplicate cash rows VOIDED (৳19,000 total) via the canonical mechanism — `transactions.type` prefixed `[VOID-DUP] ` (excluded by `notBCF` regex in Reports.jsx line 23) + mirror `payment_transactions.status='REFUNDED'`. Voided IDs: 4b36a289 (ZUBAYED 4000), d3465c35 (RAKIB 3500), 235e6281 (SI SHAMIM 2000), fa2b0cce (SOLIMAN/SARFARAJ 3500), bafa8444 (ENJAMUL 6000). Backup of originals in table `_backup_voided_cash_2026_06_13` (reversible: strip prefix + status→COMPLETED). Folios confirmed empty (no extras justified the excess). After void, SOLIMAN/RAKIB/SI SHAMIM reconcile exactly to net bill; ZUBAYED to paid_amount + a real ৳60.
- OPEN: ENJAMUL res `a5d6eac5` was REUSED for guest SAJJADUL (total 2000). One dup 6000 voided; the OTHER 6000 is ENJAMUL's REAL payment misattributed to SAJJADUL's reservation — needs RE-ATTRIBUTION to ENJAMUL's correct reservation, NOT a void. Do not void it.
...
DUPLICATE STAY-EXTENSION ROW VOIDED — ANAMUL ISLAM (2026-07-02, nightly CRM Orchestra + owner approval):
- CONTEXT: nightly audit (read-only) dup-transaction check flagged two live "Stay Extension (+1 night)" ৳6,000 rows for reservation 1887506b (ANAMUL ISLAM, room 508), both on fiscal_day 2026-06-30, created 10h apart (00:45 + 10:52 UTC). 2-night stay (06-28→06-30), total_amount ৳12,000 = 2×৳6,000, so total reflects only 2 nights → one extension row was NOT folded into the reservation total = genuine duplicate.
- SHADOW AUDIT: no live billing harm — no negative invoice, status CHECKED_OUT, paid ৳10,000 = net (total − ৳2,000 discount), due ৳0. Only downstream effect = ~৳6,000 revenue OVER-COUNT in the 2026-06-30 daily report (Stay Extension counts as a charge in revenue sums).
- GATE: classified P2 → HELD in the nightly digest (Gmail draft to ahmedshanwaz5@gmail.com; Slack #crm-alerts skipped, unauthenticated). Owner approved "void ANAMUL dup" 2026-07-02.
- FIX APPLIED TO PROD (mynwfkg): relabeled row id ea976362-2201-4c44-9571-3d85739822dd (the 10:52 row) from 'Stay Extension (+1 night)' → '[VOID-DUP] Stay Extension (+1 night)', matching the existing VOID-DUP convention so revenue matchers exclude it. UPDATE scoped to id + current type = idempotent (2nd run affected 0 rows). Did NOT touch paid_amount/due.
- VERIFIED: idempotent (0 rows on re-run); 0 live Stay-Extension dup pairs remain for the reservation; billing invariants unchanged (total ৳12,000, paid ৳10,000, due ৳0, CHECKED_OUT). Net: 06-30 daily revenue drops the phantom ৳6,000; no guest-facing balance moved.
- REVERSIBLE: set type back to 'Stay Extension (+1 night)' on that id to undo.
- WORKFLOW NOTE: qa-fix preview branch was intentionally SKIPPED — Supabase preview branches are provisioned with_data:false so they cannot hold the specific prod row (verification there would be cosmetic) and incur separate cost confirmation. For an owner-approved, idempotent, reversible single-row DML relabel, applied directly to prod with pre-image capture + pre/post verification instead.
- KEPT AS-IS: SOHAG BABU (res 0ad3fe4c, room 503) same-fiscal-day Stay-Extension pairs = FALSE POSITIVE (reconciled 10-night stay, total ৳60,080 ≈ 10×৳6,000+folio, due ৳0). Do not void if re-flagged.

DISCOUNT MIRROR TRIGGER — BIDIRECTIONAL, PERMANENT ARCHITECTURE (2026-07-10):
- RECURRENCE: scheduled backup-cleanup gate found 5 CHECKED_OUT rows with discount < discount_amount (৳3,000 phantom dues) — 4 of them CREATED 07-03→07-06, AFTER the 07-03 reconcile. The 06-30 "historical drift only" conclusion was WRONG.
- TRUE ROOT CAUSE: `sync_compat_reservation_columns()` only backfilled `discount` FROM `discount_amount` when `discount` was NULL/0. The app writes ONLY `discount_amount` (ReservationEditModal → POST /api/crm/reservation update, route line ~228; create fan-out line ~144). So any EDIT that changed an existing discount (e.g. 500→1000) left canonical `discount` stale → phantom due = the gap. INSERTs were fine; UPDATEs were the hole.
- FIX (migration `fix_discount_mirror_bidirectional_2026_07_10`, prod mynwfkg): trigger is now TG_OP-aware. INSERT = old backfill kept, made bidirectional (whichever side is set fills the other). UPDATE = mirror whichever side the writer changed (`IS DISTINCT FROM OLD`); if a writer explicitly sets BOTH to different values in one statement, respect the writer (no-op). `discount` and `discount_amount` can no longer diverge through any live path. NO app-code change needed — do NOT add discount-sync logic client-side or in routes; the trigger IS the mirror.
- REPAIR: `_backup_discount_reconcile_2026_07_10` (5-row pre-image) → `UPDATE discount=discount_amount` on the divergent CHECKED_OUT set, with the same 7 outbound triggers per-name DISABLE/ENABLE'd atomically as the 07-03 apply ("Send Confirmation Email", on_checkout_queue_review, trg_queue_review, trg_auto_referral, trg_auto_upsell, trg_auto_housekeeping, sync_reservations_to_sheets) — no guest emails/webhooks fired. (Reminder: session_replication_role is NOT settable by the MCP role on Supabase — per-name ALTER TRIGGER is the pattern.)
- VERIFIED: f1_divergent 5→0; 0 triggers left disabled; live BEGIN…ROLLBACK probe on prod confirmed an UPDATE to discount_amount now auto-mirrors discount inside the same statement (pg_net webhook queue rows rolled back with the txn).
- KEPT AS-IS: 5 pre-existing REVERSE rows (discount > discount_amount > 0). Billing reads canonical `discount`, so these carry NO phantom dues — the legacy column is merely stale. Do NOT "reconcile" them backwards; that would change real bills.
- BACKUP LIFECYCLE: the 07-03 backup tables (`_backup_discount_reconcile_2026_07_03`, `_backup_paytx_dedupe_2026_07_03`) were RETAINED by the 07-10 cleanup run (gate blocked on the recurrence). With the root cause fixed, the next scheduled cleanup run should pass and drop them; `_backup_discount_reconcile_2026_07_10` gets its own 7-day window.

MOBILE CLS 0.61 + FCP FIX -- FONT ARCHITECTURE DECISION (2026-07-11):
- SPEED INSIGHTS (mobile, 7d, 22 datapoints): FCP/LCP 3.36s, CLS 0.61, TTFB 1.6s; route "/" = 4s P75. Window mostly predates the 07-10 landing perf pass (CookieHub/LazyMotion), but CLS was never addressed by that pass.
- CLS ROOT CAUSE: Playfair Display (--font-display, incl. the text-6xl..8xl hero h1) loaded via the Google Fonts stylesheet with display=swap and a RAW Georgia fallback -- metric mismatch reflowed the entire hero (and everything below it) on font swap.
- FIX 1 (PERMANENT FONT RULE): Playfair Display is now self-hosted via next/font in app/layout.tsx (var --font-playfair, weights 500/600/700 + italic; next/font injects a size-adjusted fallback = shift-free swap). globals.css: --font-display = var(--font-playfair), 'Playfair Display', Georgia, serif. NEVER re-add Playfair to any fonts.googleapis.com URL -- that reintroduces the CLS.
- FIX 2 (STYLESHEET SCOPING RULE): the root layout no longer ships ANY Google Fonts stylesheet. The 4 remaining Google families (DM Sans / Roboto / IBM Plex Mono / Libre Baskerville) are CRM/admin-only and load via the new app/components/UiFonts.tsx (preconnect + precedence link; React 19 hoists + dedupes). Mounted in: app/crm/layout.tsx, app/billing/page.jsx, app/admin/audit/page.tsx (both returns), app/admin/onboard/page.tsx, app/lumea/page.tsx. RULE: any NEW page outside /crm that uses the Ivory/CRM design tokens must mount <UiFonts />. Public (site) pages now make ZERO Google-CSS requests (Geist/Cormorant/Playfair are all next/font).
- TTFB 1.6s -- OPEN, NOT TOUCHED: per-request CSP nonce (headers() in root layout + middleware) forces full dynamic SSR of the marketing pages. Making "/" static/ISR requires a hash-based or relaxed CSP for the (site) group = owner architecture call.
- ENV LANDMINE (this session): the F: sandbox mount served PERSISTENTLY truncated reads (app/crm/layout.tsx read as 11/17 lines even after re-copy) -> /tmp shadow tsc emitted FALSE "unterminated literal at EOF" errors in files never touched (ReservationModal, close-day-chain, middleware). Host Read tool showed every file intact. When re-copy does not clear truncation, SKIP sandbox tsc entirely: verify via host Read + let the Vercel build be the compile gate.
- DEPLOY: via the auto-PR pipeline (claude/heuristic-darwin) -- verify newest Vercel deploy builds GREEN; expect CLS/FCP recovery in Speed Insights over ~7d of RUM.

CANONICAL HOST -- SINGLE-ORIGIN 301 DECISION (2026-07-18):
- SYMPTOM: GSC (property All known pages, last update 07-10) -- 5 indexed, 1 "Duplicate without user-selected canonical" (source: Website; validation Not Started). 6 known pages = the 5 apex sitemap URLs + 1 duplicate.
- ROOT CAUSE: middleware.ts INDEXABLE_HOSTS listed fountainbd.com AND www.fountainbd.com (and hotel.fountainbd.com), with NO redirect between them. All three served identical 200 marketing content while every per-page alternates.canonical + app/sitemap.ts hardcode the bare apex https://fountainbd.com. Multiple live indexable origins + no redirect = the exact GSC "Duplicate without user-selected canonical" trigger. The 6th known page = the www homepage duplicate. (trailingSlash:false in next.config.mjs ruled out a slash variant.)
- DECISION (PERMANENT ARCHITECTURE): apex fountainbd.com is the ONE canonical marketing origin. www.fountainbd.com and hotel.fountainbd.com now 301 → apex (preserve path+query) at the top of middleware(), before the CRM perimeter gate + CSP logic. INDEXABLE_HOSTS reduced to apex only (belt-and-suspenders noindex if a request ever slips the redirect). Redirect is SCOPED to those two aliases by exact hostname match — www.lumea.fountainbd.com stays on its own Lumea rewrite branch and must NOT be caught here.
...
## 2026-08-06 — Vercel project merge + E2E retarget + Speed Insights re-scope
- **`hotelfountainbdcrm-e2e` Vercel project DELETED** (owner-approved). It auto-built every main push (duplicate builds + duplicate bot-serving lambdas) and its Supabase preview branch was already gone; its env values were Sensitive (unrecoverable). Team was over the Hobby Fluid CPU cap (4h10m/4h) — the duplicate project was part of that burn.
- **E2E now targets the LIVE deployment via the dedicated test tenant `lumeademo`** (`156da579`, slug host `https://lumeademo.lumea.fountainbd.com`): rooms expanded to D101–D105, `demo-owner@lumea.invalid` password reset (bcrypt-12, session_v bumped). Isolation = tenant_id RLS; the suite seeds + cascade-deletes its own data and doubles as a tenant-isolation canary. Never point E2E at the Fountain prod tenant.
- e2e.yml: `E2E_BASE_URL` defaults to the lumeademo URL (secrets override — DELETE the stale secret pointing at the retired project). Required secrets now only `E2E_EMAIL`/`E2E_PASSWORD`. `fixtures.ts` TEST_ROOMS default = D101–D105 (was prod-shaped 101–205 — dangerous default removed).
- **Speed Insights**: collection was never broken — the 19–20s desktop TTFB/FCP/LCP reds were Jul-30-era pre-fix samples aging out of the 7-day window (last-24h desktop TTFB = 1.1s). Real gap: /crm unsampled since 4d10257. `<SpeedInsights/>` added to `app/crm/layout.tsx` — safe under the static-shell 'unsafe-inline' CSP (ee5aad3). Root layout + strict surfaces stay telemetry-free (4d10257 rule unchanged).
