...
- NOT DONE / OPEN (needs owner call): `execute_nightly_audit()` itself was NOT changed. Its `total_collections` column stays wrong-by-charges, and Reports.jsx (~L320) surfaces it as the "Closed Collection · locked at close" StatCard — so that card still reads ৳63,000 for 2026-08-07. Fixing the RPC means re-defining a persisted money column and would make historical night_audit_log rows inconsistent with new ones; that is a P1/P2-class change requiring explicit owner approval + a backfill decision. Flagged to owner 2026-08-08.
