...
## 2026-06-12 Reports - Outstanding Dues as full table (feat)
- Per owner: the print report's Outstanding Dues block changed from the compact inline footer to a full table matching Daily Movements - columns Guest | Room | Status | Balance Due, maps ALL `allDue` rows + a bold `Total Outstanding - N reservations` total row (`.pr-tot-row`). Reuses `.pr-tbl` print styles.
...
## 2026-06-12 CANONICAL: "Stay Extension" is NOT an extra charge (near-miss, DO NOT re-fix)
- During a "is Due Balance correct?" audit I FIRST wrongly concluded HUMAYON KABIR (510) and MOHAMMAD RAIHAN (308) were understated because each had a `transactions` row typed "Stay Extension (+N nights)" (৳8,000 / ৳4,500) not present in `total_amount`. Adding them would have DOUBLE-COUNTED ~৳12,500 (ghost-bleed) - caught by Shadow Audit before any write.
- CANONICAL TRUTH: `recalcResTotal` / `recalcResTotalServer` set `total_amount = SUM(room.price x nights) + SUM(billable folios)`, with `nights` derived from check_out. A stay extension PUSHES check_out, so recalc folds the extra nights into `total_amount` via room x nights. The "Stay Extension" TRANSACTION row is an INFORMATIONAL, payment-excluded (notBCF) record of the event - NOT an additional receivable. Verified: HUMAYON total 12,000 = 3 nights x 4,000 (incl. extension); 308 total 9,000 = 2 x 4,500 (incl. extension). Both balances correct.
- RULE (do not violate): due = max(0, total_amount - discount - paid). NEVER add Stay-Extension/charge transactions on top of total for due. "Add Charge" (Room Service etc.) writes a FOLIO and recalc already includes billable folios; stay extensions ride via nights. Both are already inside total_amount whenever recalc has run.

## 2026-06-12 Check-in/out blocked by expired session + dead anon fallback (fix)
- SYMPTOM: Check-Out modal showed "permission denied for table reservations"; console = `/api/crm/check` 401 + a Supabase REST 401. ROOT: `lumea_sess` cookie is HMAC-signed with a 12h max-age (src/lib/session.ts MAX_AGE_S); after ~12h `requireSession` returns null so EVERY Phase-3 write route 401s. The modals then fall back to a DIRECT ANON write which RLS hardening revoked - permission denied (misleading).
- IMMEDIATE FIX: re-login (mints fresh 12h session) - unblocks check-in/out and all writes.
- CODE FIX (commit cb171f4dc374eec14afbdd5ba8d48d8c7e8475b6, cherry-picked to main): CheckActionModal now throws "Your session has expired. Please sign out and sign in again" on 401 and DROPS the dead anon fallback (removed getSupabaseClient import). SAME dead-fallback pattern still in AddChargeModal + ReservationEditModal (+ likely RecordPaymentModal/NewReservationModal) - sweep TODO. Consider lengthening session or adding silent refresh so front-desk isn't logged out mid-shift.

## 2026-06-12 Sliding session (silent refresh) - no more mid-shift re-login (feat)
- Replaced the hard 12h `lumea_sess` expiry with a 7-day SLIDING window (src/lib/session.ts MAX_AGE_S). New GET `/api/crm/session` re-issues the HttpOnly cookie (honors session_v / Logout-All; 401 if revoked). AuthGate pings it on mount + every 20 min + on tab focus, so an open front-desk tab never lapses; a true 401 signs out cleanly to the login (no more mid-action "permission denied"). Only ~7-day fully-idle sessions require re-login.
- Pairs with prior CheckActionModal fix (clear session message). Same dead anon-write fallback still TODO in AddChargeModal/ReservationEditModal/RecordPaymentModal. Commit 5fcc97e90f411604510e14e3555780b824cfde9b cherry-picked to main.

## 2026-06-12 (run 3) CEO nightly sweep — CLEAN, no writes
- 2026-06-12 (run 3) CEO sweep: clean — 0 actionable findings, 0 writes. Integrity GREEN (0 neg / 0 orphans / 0 dup-keys), advisors 0 ERROR, auth empty, edge+API all 200. Branch-QA confirmed restored (main `FUNCTIONS_DEPLOYED`, no leftover branches) but not exercised — no candidate fix. 3 isolated `column does not exist` postgres errors (`rooms.name` / `marketing_content.scheduled_date` / `daily_closing.date`, 05:55 UTC) = ad-hoc SQL probe (no REST correlation), Info/no-action. Run-2 P3 dup-index fix skipped (idempotent). Prior `tenant_isolation` always-true P2 on `customers`/`fountain_inventory` re-verified CLOSED (no always-true ALL policy remains). Report: `lumea-agents/CEO_SWEEP_REPORT_2026-06-12_run3.md`.

## 2026-06-12 Swept dead anon-write 401 fallbacks from write modals (fix)
- Removed the leftover "on 401 -> direct anon write" fallback from AddChargeModal, RecordPaymentModal, ReservationEditModal (RLS revoked anon writes - produced "permission denied"). All three now throw a clear "Your session has expired - sign in again" on 401, matching CheckActionModal. With the new sliding session this should rarely trigger. Commit 936f43c95b1874dd27c342fd06187abcfbcadcca cherry-picked to main.
