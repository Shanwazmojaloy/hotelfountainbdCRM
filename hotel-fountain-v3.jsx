    ...
    <div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:12,marginBottom:15}}>
        {[{l:"Total",v:filtered.length,c:"#58A6FF"},{l:"Revenue",v:BDT(filtered.reduce((a,r)=>a+r.amount,0)),c:"#C8A96E"},{l:"Avg Value",v:filtered.length?BDT(Math.round(filtered.reduce((a,r)=>a+r.amount,0)/filtered.length)):"৳0",c:"#2EC4B6"}].map(s=>(
          <div key={s.l} className="card"><div className="card-body" style={{padding:12}}>
            <div className="xs muted mono mb2" style={{letterSpacing:".1em",textTransform:"uppercase"}}>{s.l}</div>
            <div className="serif" style={{fontSize:22,fontWeight:700,color:s.c}}>{s.v}</div>
          </div></div>
        ))}
      </div>
      ...
      {showAdd&&(
        <Modal title="Add Task" onClose={()=>setShowAdd(false)}
          ...
          <div className="frow">
            <div className="fg"><label className="flbl">Room *</label>
              <select className="fselect" value={form.room} onChange={e=>setForm(p=>({...p,room:e.target.value}))}>
                <option value="">Select...</option>{rooms.map(r=><option key={r.id} value={r.id}>{r.id} ({r.status})</option>)}
              </select>
            </div>
        ...
        <div>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:12,marginBottom:15}}>
            {[{l:"Transactions",v:daily.length,c:"#58A6FF"},{l:"Revenue",v:BDT(rev),c:"#C8A96E"},{l:"Avg",v:daily.filter(t=>t.status==="completed").length?BDT(Math.round(rev/Math.max(1,daily.filter(t=>t.status==="completed").length))):"৳0",c:"#2EC4B6"},{l:"Pending",v:BDT(daily.filter(t=>t.status==="pending").reduce((a,t)=>a+t.amount,0)),c:"#E05C7A"}].map(s=>(
              <div key={s.l} className="stat" style={{"--ac":s.c}}><div className="stat-lbl">{s.l}</div><div className="stat-val" style={{fontSize:24,color:s.c}}>{s.v}</div></div>
            ))}
          </div>
