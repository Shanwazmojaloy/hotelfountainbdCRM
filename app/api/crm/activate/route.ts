...
// /api/crm/send-otp emailed (SHA-256 hash, ≤5 min), then sets the password (pwh) and marks
// the account activated — all on the SERVICE ROLE (anon can no longer write staff). On
// success returns a minimal session so the client can sign the user straight in.
