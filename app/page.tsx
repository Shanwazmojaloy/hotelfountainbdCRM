    ...
    try {
      ...
      if (existing?.id) {
        ...
        const { data: guestData, error: guestError } = await supabase
          .from('guests')
          .insert([{ name: bookForm.name, email: bookForm.email.trim().toLowerCase(), phone: bookForm.phone, address: bookForm.address || null, tenant_id: '46bbc3ff-b1ef-4d54-87be-3ecd0eb635a8', total_stays: 0, total_spent: 0, loyalty_points: 0, outstanding_balance: 0, vip: false }])
          .select('id').single();
      ...
      <section id="experience" className="sec" style={{ padding: '100px 60px', background: 'var(--dark2)' }}>
        ...
        <div className="exp-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 60, alignItems: 'start' }}>
          ...
          <div className="exp-visual" style={{ position: 'relative', overflow: 'hidden', border: '1px solid var(--br)' }}>
            ...
            <iframe src="https://www.google.com/maps/embed?pb=!1m18!1m12!1m3!1d3649.7005234745966!2d90.4162137758988!3d23.82924608574381!2m3!1f0!2f0!3f0!3m2!1i1024!2i768!4f13.1!3m3!1m2!1s0x3755c72983411885%3A0xe650b3a66c15269a!2sHotel%20Fountain!5e0!3m2!1sen!2sbd!4v1775873778238!5m2!1sen!2sbd" width="100%" height="420" style={{ border: 0, display: 'block', filter: 'invert(.9) hue-rotate(180deg) saturate(0.6) brightness(0.85)' }} allowFullScreen loading="lazy" referrerPolicy="no-referrer-when-downgrade" title="Hotel Fountain Location" />
        ...
        <div className="gc" style={{ maxWidth: 860, padding: 48, position: 'relative', zIndex: 1 }}>
          <div className="avail-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 20, marginBottom: 32 }}>
            {[
              ...
              { label: 'Room Type', el: <select className="fi gs" value={roomType} onChange={e => setRoomType(e.target.value)}><option value="">Any Room</option>{AVAIL_ROOMS.map(r => <option key={r.value} value={r.value}>{r.value}</option>)}</select> },
              { label: 'Guests', el: <select className="fi gs" value={guests} onChange={e => setGuests(e.target.value)}><option>1 Guest</option><option>2 Guests</option><option>3 Guests</option><option>4 Guests</option><option>5 Guests</option><option>6 Guests</option></select> },
            ].map(({ label, el }) => (
      ...
      <section id="terms" className="sec" style={{ padding: '80px 60px', background: 'var(--dark)', borderTop: '1px solid var(--br)' }}>
        ...
        {tncOpen && (
          <div className="afi tnc-cols" style={{ columns: 2, columnGap: 48, marginTop: 32 }}>
            {[
              { num: '1', title: 'Booking and Reservations', items: [{ h: 'Eligibility', b: 'Guests must be at least 18 years of age to book a room.' }, { h: 'Identification', b: "A valid government-issued photo ID (NID, Passport, or Driver's License) must be presented by all guests at check-in." }, { h: 'Right of Refusal', b: 'Hotel Fountain reserves the right to refuse service or accommodation to anyone who violates hotel policies or disrupts the peace and safety of the property.' }] },
              { num: '2', title: 'Pricing, Payments & Zero-Discount Policy', items: [{ h: 'Currency', b: 'All rates, incidentals, and fees are quoted and charged in Bangladeshi Taka (BDT).' }, { h: 'Strict Pricing Policy', b: 'All displayed and quoted room rates are final. Hotel Fountain enforces a strict zero-discount policy across all bookings. No employee, including front desk officers, is authorised to alter the standard room rate or provide complimentary upgrades.' }, { h: 'Payment Terms', b: 'Full payment for the room rate is required at check-in or upon booking. A token amount or incidental deposit may also be collected.' }, { h: 'Additional Charges', b: 'Any incidental expenses incurred during the stay (e.g., room service, laundry, damages) will be billed as separate "Add Charges" on the final invoice and must be settled in full prior to check-out.' }] },
              { num: '3', title: 'Check-In and Check-Out Policies', items: [{ h: 'Standard Check-In Time', b: '12:00 PM' }, { h: 'Standard Check-Out Time', b: '12:00 PM' }, { h: 'Early / Late', b: "Subject to room availability and may incur additional fees. Failure to check out by the designated time without prior management approval will result in a late checkout fee or an additional night's charge." }] },
              { num: '4', title: 'Cancellations and No-Shows', items: [{ h: 'Standard Cancellation', b: 'Reservations must be cancelled at least 48 hours prior to the scheduled check-in date to receive a full refund of any deposits.' }, { h: 'Late Cancellation / No-Show', b: 'Cancellations made past the permitted window or failure to arrive for a reservation will result in a penalty additional charge.' }] },
              { num: '5', title: 'Property Rules & Guest Conduct', items: [{ h: 'Damage Policy', b: "Guests are fully liable for any damages caused to hotel property, furnishings, or equipment by themselves or their visitors. Repair or replacement costs will be charged directly to the guest's folio." }, { h: 'Smoking Policy', b: 'Smoking is strictly prohibited inside all hotel premises. A cleaning fee will be charged for violations.' }, { h: 'Outside Visitors', b: 'For security purposes, all non-registered visitors are not allowed to rooms.' }] },
              { num: '6', title: 'Limitation of Liability', items: [{ h: 'Disclaimer', b: 'Hotel Fountain is not liable for the loss, theft, or damage of personal belongings, valuables, or vehicles brought onto the property by guests.' }] },
              { num: '7', title: 'Governing Law', items: [{ h: 'Jurisdiction', b: 'These terms and conditions are governed by and construed in accordance with the laws of Bangladesh. Any disputes arising from these terms shall be subject to the exclusive jurisdiction of the courts in Dhaka.' }] },
            ].map(sec => (
              <div key={sec.num} style={{ breakInside: 'avoid', marginBottom: 28, pageBreakInside: 'avoid' }}>
                <div className="gs" style={{ fontSize: 10, letterSpacing: '.18em', textTransform: 'uppercase', color: 'var(--gold)', marginBottom: 10 }}>{sec.num}. {sec.title}</div>
                {sec.items.map(item => (
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                  {[
                    ...
                    <div key={f.key} style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                      ...
                      <input type={f.type} className="fi gs" placeholder={f.ph} value={(bookForm as Record<string, string>)[f.key]} onChange={e => setBookForm(p => ({ ...p, [f.key]: e.target.value }))} />
                    </div>
                  ))}
                </div>
