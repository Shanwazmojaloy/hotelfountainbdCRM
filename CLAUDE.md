...
- **BOOKING CONFIRMATION PRINT — WHATSAPP QR (2026-06-03)**: `printConfirmation()` footer is two-column with `align-items:flex-end` — left side has phone+email+web, right side has 80×80 WhatsApp QR code (api.qrserver.com → wa.me/8801322840799) + "SCAN TO WHATSAPP" label + monospace phone number. Print trigger is image-load-aware: counts all `<img>` `load`/`error` events and only calls `window.print()` once all images settle (+120ms buffer), with a 2.5s hard cap. NEVER revert to fixed `setTimeout(()=>print(), 350)` — QR sometimes hadn't fetched yet. Same QR pattern applied to `printInvoice` previously (commit `a6f39810`).
