...
- **BILLING REPORT — A4 PORTRAIT + READABLE (2026-06-02)**: Download Report uses `@page{size:A4 portrait;margin:5mm 7mm}`. Owner-confirmed readability bump on 2026-06-02 — body 9px, h1 17px, stat val 18px/700w, sec-hdr 12.5px/700w, table th 9px/700w, table td 10px, closing final 14px (৳ value 18px), pm pills 8.5px/600w. Print stylesheet includes `text-rendering:geometricPrecision`, `font-feature-settings:'tnum' 1, 'lnum' 1`, darker `color:#15110D` for body. If overflow to page 2 with 25+ guests, prefer trimming columns over shrinking fonts; do not revert to landscape.
- **DOWNLOAD REPORT NUMBERS MUST MATCH WEB BILLING & INVOICES (2026-06-02)**: PDF "Bill Total" column = `computeBill(r).total` (NET, post-discount) in BOTH the Collected Transactions and Pending Dues sections — NEVER raw `res.total_amount` (gross). PDF Collected Transactions "Paid" column = `+tx.amount||0` (today's collected per tx, mirrors web's filter=TODAY behavior) — NEVER lifetime `bill.paid`. Pending Dues "Paid" stays as lifetime `bill.paid` because the user wants to see partial historical payments against the outstanding balance. Bug source: previously `bill_total: bill ? ((+res.total_amount||0) || bill.sub) : 0` showed gross; web showed net via `computeBill.total`. Fixed at lines ~3355 and ~3371 in `public/crm-src.jsx`.
