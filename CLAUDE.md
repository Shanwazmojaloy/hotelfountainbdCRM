...
- **BOOKING CONFIRMATION PRINT — WHATSAPP QR + SINGLE-PAGE (2026-06-03)**: `printConfirmation()` uses `@page{size:A4 portrait;margin:6mm 8mm}` and a comprehensive `@media print` block that compresses every section so the QR-bearing footer stays on page 1. Key sizes (print only): body 11px, h1 18px, doc-title 16px, doc-sub 9px, .grid gap 12px, .box padding 9×12, table th/td 7×9 (font 8.5/10.5), .totals 260px width with row 11px/bal 12.5px, .terms margin-top 12px/font 8.5px, .ftr margin-top 14px/font 9px. QR is 64×64 in print (img rendered at 120px source for sharpness), positioned right with `align-items:flex-end`. Print trigger is image-load-aware: counts all `<img>` `load`/`error` events and only calls `window.print()` once all images settle (+120ms buffer), with a 2.5s hard cap. NEVER revert to fixed `setTimeout(()=>print(), 350)` — QR sometimes hadn't fetched yet. Same QR pattern + single-page rule applied to `printInvoice` previously.
