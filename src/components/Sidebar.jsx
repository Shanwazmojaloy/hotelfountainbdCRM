...
export default function Sidebar() {
  ...
  return (
    <aside className="iv-sidebar hidden md:flex flex-col w-64 h-screen sticky top-0" style={{ overflow: 'hidden' }}>
      ...
      {/* User footer */}
      <div style={{ padding: '14px 16px', borderTop: '1px solid rgba(200,169,110,.12)', flexShrink: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ fontSize: 12, fontWeight: 500, color: 'var(--iv-ink-inv, #EEE8DC)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{user?.name || 'Staff'}</div>
            <div style={{ fontSize: 8, color: 'var(--iv-gold-light)', letterSpacing: '.1em', textTransform: 'uppercase', marginTop: 1 }}>{user?.role || ''}</div>
          </div>
          <span onClick={signOut} title="Sign out" style={{ fontSize: 14, color: 'rgba(200,169,110,.5)', cursor: 'pointer', padding: 4 }}>⎋</span>
        </div>
      </div>
    </aside>
  );
