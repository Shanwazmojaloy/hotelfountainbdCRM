...
/* ── LAYOUT ── */
.app{display:flex;height:100vh;height:100dvh;overflow:hidden}.lm-burger{display:none}.lm-scrim{display:none}@media(max-width:768px){.sidebar{position:fixed!important;top:0;left:0;bottom:0;z-index:1200!important;width:266px!important;max-width:86vw;transform:translateX(-100%);transition:transform .28s cubic-bezier(.4,0,.2,1);box-shadow:6px 0 44px rgba(0,0,0,.6)}body.nav-open .sidebar{transform:translateX(0)!important}.lm-scrim{display:block!important;position:fixed;inset:0;background:rgba(0,0,0,.55);z-index:1100;opacity:0;pointer-events:none;transition:opacity .28s}body.nav-open .lm-scrim{opacity:1;pointer-events:auto}.lm-burger{display:flex!important;align-items:center;justify-content:center}.main{width:100%!important;min-width:0}.topbar{height:52px!important;padding:0 12px 0 56px!important;gap:8px!important}.tb-title{font-size:15px!important;line-height:1.1!important}.tb-meta{display:none!important}.content{padding:14px 12px!important}.stats,.stats-row{grid-template-columns:repeat(2,1fr)!important;gap:8px!important}.grid{grid-template-columns:1fr!important;gap:14px!important}.rooms-grid{grid-template-columns:repeat(2,1fr)!important;gap:8px!important}.tbl-wrap{overflow-x:auto!important;-webkit-overflow-scrolling:touch}.tbl{min-width:640px}}
...
.content{flex:1;overflow-y:auto;min-height:0;padding:20px 24px;background:var(--bg)}
...
/* ── MODALS ── */
.modal-bg{position:fixed;inset:0;min-height:100%;min-height:100dvh;background:rgba(0,0,0,.85);display:flex;align-items:center;justify-content:center;z-index:1000;padding:20px;backdrop-filter:blur(2px);overflow-y:auto}
.modal{background:var(--s2);border:1px solid var(--br);border-top:4px solid var(--gold-light);width:100%;max-width:540px;max-height:92vh;max-height:92dvh;overflow-y:auto;background:#1C1510;box-shadow:0 24px 80px rgba(28,21,16,.25);animation:mIn .2s ease}
...
.login-bg{min-height:100vh;display:flex;flex-direction:row;overflow:hidden;background:#100E0B}@media(max-width:768px){.login-bg{flex-direction:column;height:100vh;height:100dvh;overflow-y:auto;overflow-x:hidden;-webkit-overflow-scrolling:touch}.lp-left{display:none}.lp-vdivide{display:none}.lp-right{flex:1 0 auto;width:100%;min-height:100vh;min-height:100dvh;padding:84px 16px 72px;align-items:flex-start;justify-content:flex-start}.lp-right-top{padding:16px 18px}.lp-right-meta,.lp-right-ver{font-size:8px}.login-card{max-width:100%;width:100%;padding:30px 20px 26px;margin:0 auto}.role-grid{gap:6px}.rpill{padding:13px 5px 11px}.rpill .ri{font-size:16px}.rpill .rl{font-size:7.5px}}
/* ── LEFT PANEL ── */
...
<style>
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  @media print{
    ...
    html,body{background:#fff !important;-webkit-print-color-adjust:exact;print-color-adjust:exact;font-size:12px !important;line-height:1.5 !important;text-rendering:geometricPrecision !important;font-feature-settings:'tnum' 1, 'lnum' 1 !important;color:#1F1B16 !important}
    /* Page is a flex column with full A4 height — footer pushes to bottom via margin-top:auto */
    ...
    /* Header — balanced (slightly larger than the over-tight v1) */
    .hdr{padding-bottom:16px !important;margin-bottom:20px !important;gap:16px !important}
    .brand img.logo{width:54px !important;height:54px !important}
    .brand h1{font-size:21px !important;letter-spacing:.5px !important}
    .brand .tag{font-size:11px !important;margin-top:3px !important;letter-spacing:1.2px !important}
    .brand .contact{font-size:9.5px !important;margin-top:5px !important;line-height:1.45 !important}
    .meta{font-size:10.5px !important;line-height:1.5 !important}
    .meta .conf{font-size:11.5px !important;font-weight:600 !important}
    /* Doc title */
    .doc-title{font-size:18px !important;margin-bottom:4px !important;letter-spacing:.4px !important;font-weight:600 !important}
    .doc-sub{font-size:9.5px !important;letter-spacing:1.6px !important;margin-bottom:22px !important}
    /* Guest/Confirmation/Date boxes */
    .grid{gap:14px !important;margin-bottom:20px !important}
    .box{padding:12px 14px !important;border-width:1px !important;border-radius:2px !important}
    .lbl{font-size:8.5px !important;letter-spacing:1.6px !important;margin-bottom:5px !important}
    .val{font-size:13px !important;font-weight:600 !important}
    .val.mono{font-size:12.5px !important;font-weight:600 !important}
    /* Room table */
    table{margin-bottom:18px !important;border-width:1px !important}
    thead th{padding:9px 11px !important;font-size:9px !important;letter-spacing:1.6px !important;font-weight:700 !important}
    tbody td{padding:10px 11px !important;font-size:11.5px !important}
    /* Totals box */
    .totals{width:290px !important}
    .totals .row{padding:5px 0 !important;font-size:12px !important}
    .totals .row.bal{margin-top:5px !important;padding-top:10px !important;font-size:14px !important;font-weight:700 !important}
    /* Terms (small print) */
    .terms{margin-top:22px !important;font-size:9.5px !important;line-height:1.55 !important}
    .terms h4{font-size:10px !important;margin-bottom:6px !important;letter-spacing:1.6px !important;font-weight:700 !important}
    /* Footer: margin-top:auto pushes it to the bottom of the A4 page.
       align-items:center vertically balances the left text against the QR. */
    .ftr{margin-top:auto !important;padding-top:14px !important;font-size:9.5px !important;letter-spacing:1.1px !important;border-top-width:.8px !important;color:#5A544A !important;align-items:center !important}
                  ...
                  {displayList.length===0?(
                    ...
                    return (
                      <tr key={r?r.id:(grp.guest_name+'|'+grp.room_number)}>
                        <td className="xs" style={{verticalAlign:'top',paddingTop:6}}>{gname}</td>
                        <td style={{verticalAlign:'top',paddingTop:6}}><span className="badge bb">{rno}</span></td>
                        <td className="xs muted" style={{verticalAlign:'top',paddingTop:6}}>{chkIn}</td>
                        <td className="xs muted" style={{verticalAlign:'top',paddingTop:6}}>{chkOut}</td>
                        <td className="xs gold" style={{verticalAlign:'top',paddingTop:6}}>{BDT(tTotal)}</td>
                        <td className="xs" style={{color:'var(--amb)',verticalAlign:'top',paddingTop:6}}>{tDiscount>0?'− '+BDT(tDiscount):'—'}</td>
                        <td className="xs" style={{color:'var(--grn)',verticalAlign:'top',paddingTop:6}}>{BDT(tPaid)}</td>
                        <td className="xs" style={{color:tDue>0?'var(--rose)':'var(--grn)',fontWeight:tDue>0?600:400,verticalAlign:'top',paddingTop:6}}>{BDT(tDue)}</td>
                        <td className="xs" style={{lineHeight:1.6,verticalAlign:'top',paddingTop:6}}>
                          {Object.keys(byType).length === 0 ? <span className="muted xs" style={{fontSize:10}}>—</span> :
                            Object.entries(byType).map(([tp,amt])=>(
                            <div key={tp} style={{display:'flex',alignItems:'center',gap:6,marginBottom:3,whiteSpace:'nowrap'}}>
                              <span className="badge bgold" style={{fontSize:8,flexShrink:0}}>{tp}</span>
                              <span style={{color:'var(--gold)',fontWeight:600,fontSize:10}}>{BDT(amt)}</span>
                            </div>
                          ))}
                        </td>
                        <td style={{whiteSpace:'nowrap',verticalAlign:'top',paddingTop:6}}>
                          {currentUser?.role!=='housekeeping'&&(
...
function RecordPayModal({toast,onClose,reload,prefill,reservations,guests,businessDate,billDue,computeBill}) {
  ...
  // fiscal_day always defaults to today's businessDate. Owner-confirmed 2026-06-02:
  // payments collected today should appear in today's BIZ DAY total regardless of
  // when the stay was checked out. User can still manually pick a past date in the
  // modal's date field if back-dating a payment is needed.
  const _smartFiscalDay = r => (businessDate||todayStr())
